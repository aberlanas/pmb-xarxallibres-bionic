...
// +-------------------------------------------------+
// $Id: ajax_selector.php,v 1.91.2.8 2018-01-25 16:04:28 apetithomme Exp $
...
switch($completion):
	...
	case 'titre_uniforme':
		...
		while($row = pmb_mysql_fetch_object($result)) {
			$authority_instance_from_selector = new authority(0, $row->tu_id, AUT_TABLE_TITRES_UNIFORMES);
			$array_selector[$row->tu_id] = $authority_instance_from_selector->get_object_instance()->get_isbd();
		}
		$origine = "ARRAY" ;
		break;		
	case 'collections':
	...
	case 'oeuvre_event':
		...
		break;
	case 'vedette':
		$requete = "select label, id_vedette from vedette where label like '".addslashes($start)."%'";
		if($linkfield) {
		...
		if (is_array($array_selector) && count($array_selector)) {
			...
			while(list($index, $value) = each($array_selector)) {
				if ($index && !is_numeric($index)) {
					$index = onto_common_uri::get_id($index);
				}
				$lib_liste="";
				if(isset($array_prefix[$index]['libelle'])) {
