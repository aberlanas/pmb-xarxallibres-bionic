...
// +-------------------------------------------------+
// $Id: cart.inc.php,v 1.43 2017-05-12 15:15:57 dgoron Exp $
...
switch ($action) {
	...
	case 'del_item':
		...
		$myCart->aff_cart_objects("./catalog.php?categ=search&mode=3&idcaddie=$idcaddie", true, 0, true);
		break;
	case 'valid_new_cart':
		$myCart = new caddie(0);
	...
	default:
		if($idcaddie) {
			...
			//aff_cart_notices($myCart->get_cart(), $myCart->type, $idcaddie);
			$myCart->aff_cart_objects("./catalog.php?categ=search&mode=3&idcaddie=$idcaddie", true, true, true);
