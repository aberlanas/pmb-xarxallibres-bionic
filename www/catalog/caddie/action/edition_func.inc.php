...
// +-------------------------------------------------+
// $Id: edition_func.inc.php,v 1.50.2.1 2017-08-23 14:41:14 jpermanne Exp $
				...
				for($i=0; $i < $nbr_lignes; $i++) {
					...
					for($iaut=0; $iaut < $max_aut; $iaut++) {
						if (isset($authors[$iaut])) {
							$worksheet->write_string($debligne_excel,($iaut*6+1+$nbr_champs),$authors[$iaut]['name']);
							$worksheet->write_string($debligne_excel,($iaut*6+2+$nbr_champs),$authors[$iaut]['rejete']);
							$worksheet->write_string($debligne_excel,($iaut*6+3+$nbr_champs),$authors[$iaut]['date']);
							$worksheet->write_string($debligne_excel,($iaut*6+4+$nbr_champs),$authors[$iaut]['function']);
							$worksheet->write_string($debligne_excel,($iaut*6+5+$nbr_champs),$authors[$iaut]['lib_type_aut']);
							$worksheet->write_string($debligne_excel,($iaut*6+6+$nbr_champs),$authors[$iaut]['lib_resp_type']);
						}
					}
...
function get_authors_editions($id_notice) {
	...
	if ($res_aut && pmb_mysql_num_rows($res_aut)) {
		...
		while ($row = pmb_mysql_fetch_object($res_aut)) {
			...
			$aut['function'] = $codes_auteurs[$row->responsability_fonction];
