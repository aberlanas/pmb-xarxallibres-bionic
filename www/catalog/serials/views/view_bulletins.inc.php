...
// +-------------------------------------------------+
// $Id: view_bulletins.inc.php,v 1.22 2017-06-22 10:19:48 dgoron Exp $
...
$bulletins .= "
		<tr>
			...
			<th><center>".$msg['bul_articles']."</center></th>
			<th><center>".$msg['bul_docnum']."</center></th>
			<th><center>".$msg['bul_exemplaires']."</center></th>";
			
if ($pmb_collstate_advanced) {
	$bulletins .= "<th><center>".$msg['bul_collstate']."</center></th>";
}
...
$bulletins .= "
		...
		<tr>
			<th align='left'>
				...
				<img src='./images/basket_small_20x20.gif' align='middle' alt='basket' title='".$msg[400]."' onClick=\"
					...
					'cart', 600, 700, -2, -2, 'toolbar=no, dependent=yes, resizable=yes, scrollbars=yes')\">
			</th>
			<th>
...
if((pmb_mysql_num_rows($myQuery))) {
	...
	while(($bul = pmb_mysql_fetch_object($myQuery))) {
		...
		$bulletins .= "</a></td><td><center>" ;
		if ($bulletin->nb_analysis) {
			$bulletins .= $bulletin->nb_analysis."&nbsp;<img src='./images/basket_small_20x20.gif' align='middle' alt='basket' title='".$msg[400]."' onClick=\"openPopUp('./cart.php?object_type=BULL&item=".$bulletin->bulletin_id."&what=DEP', 'cart', 600, 700, -2, -2, 'toolbar=no, dependent=yes, resizable=yes, scrollbars=yes')\">"; 
		}
		else {
			$bulletins .= "&nbsp;";
		}
		$bulletins .= "</center></td><td>".$href_start."<center>";
		if (sizeof($bulletin->nbexplnum)) {
			$bulletins .= $bulletin->nbexplnum; 
		}else {
			$bulletins .= "&nbsp;";
		}
		$bulletins .= "</center></a></td><td>".$href_start."<center>";
		if (sizeof($bulletin->expl)) {
			$bulletins .= sizeof($bulletin->expl); 
		} else {
			$bulletins .= "&nbsp;";
		}
		$bulletins .= "</center></a></td>";
		if ($pmb_collstate_advanced) {
			$bulletins .= "<td>";
			$collstate_list = "";
