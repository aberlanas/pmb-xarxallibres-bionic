...
// +-------------------------------------------------+
// $Id: commandes.inc.php,v 1.91.2.1 2017-10-11 12:58:52 jpermanne Exp $
...
function show_list_cde($id_bibli, $id_exercice = 0) {
	...
	if(!isset($sortBy)){
		if(isset($_SESSION["acquisition_cde_sort"]) && trim($_SESSION["acquisition_cde_sort"])){
			...
			if(count($sugg_origins)){
				...
				foreach($sugg_origins as $array_origin_details){
					if($i == 0){ //Premier tour !
						...
						$others_applicants = str_replace('!!nb!!', $i, $others_applicants);
