...
// +-------------------------------------------------+
// $Id: cms_module_metadatas_datasource_metadatas_record.class.php,v 1.3.2.2 2018-01-15 10:06:34 ngantier Exp $
	...
	*/
	...
	protected function get_record_content($notice_class) {
		...
		if($this->parameters['used_template']){
			...
			$content = $tpl->build_notice($notice_class->notice_id);
		}else{
			if($opac_notices_format == AFF_ETA_NOTICES_TEMPLATE_DJANGO){
				...
				$content .= record_display::get_display_extended($notice_class->notice_id);
			}else {
