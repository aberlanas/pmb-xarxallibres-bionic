...
// +-------------------------------------------------+
// $Id: selector_titre_uniforme.class.php,v 1.4.2.1 2017-11-07 11:18:49 vtouchard Exp $
...
class selector_titre_uniforme extends selector_authorities {
	...
	protected function get_display_object($authority_id=0, $object_id=0) {
		...
		$display = '';
		$authority = new authority($authority_id, $object_id, AUT_TABLE_TITRES_UNIFORMES);
		$titre_uniforme = $authority->get_object_instance();
		
		$display .= "<div class='row'>";
		$display .= pmb_bidi($authority->get_display_statut_class_html()."<a href='#' onclick=\"set_parent('$caller', '".$authority->get_num_object()."', '".htmlentities(addslashes($authority->get_isbd()),ENT_QUOTES, $charset)."','$callback')\">".$authority->get_isbd()."</a>");
		$display .= "</div>";
		return $display;
	}
