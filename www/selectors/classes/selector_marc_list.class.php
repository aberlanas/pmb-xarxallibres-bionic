...
// +-------------------------------------------------+
// $Id: selector_marc_list.class.php,v 1.2.2.1 2017-12-08 09:35:06 jpermanne Exp $
	...
	protected function get_display_list() {
		...
		if ($favorite) {
			if ($letter!='Fav') {
				...
				$display_list .= "<font size='+1'><strong><u>".$msg['favoris']."</u></strong></font> ";
			}
		}
		if ($special) {
			if ($letter!='My')
				...
				$display_list .= "<font size='+1'><strong><u>#</u></strong></font> ";
		}
		foreach($alphabet as $dummykey=>$char) {
			...
			if(sizeof($present) && strcasecmp($letter, $char))
				...
				$display_list .= "<font size='+1'><strong><u>$char</u></strong></font> ";
		}
		$display_list .= "</div><hr />";
		$display = array();
