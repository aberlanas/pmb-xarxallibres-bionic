...
// +-------------------------------------------------+
// $Id: index_concept_form.tpl.php,v 1.7.4.1 2018-01-19 14:21:07 dgoron Exp $
...
<script type='text/javascript'>
	...
	function onto_set_display_label(element_name,element_order, new_order, node) {
		if(node.nodeName == 'SPAN') {
			var current_child = node;
			
			for(var j in current_child.childNodes){
				if(current_child.childNodes[j].nodeName == 'SPAN') {
					onto_set_display_label(element_name,element_order, new_order, current_child.childNodes[j]);
				} else {
		...
		if(new_child){
			...
			for(var i in new_child.childNodes){
				if(new_child.childNodes[i] != undefined && new_child.childNodes[i].nodeType == Node.ELEMENT_NODE){
					...
					onto_set_display_label(element_name,element_order, new_order, new_child.childNodes[i]);
