...
// +-------------------------------------------------+
// $Id: fields_empr.inc.php,v 1.163.2.11 2018-02-02 11:21:11 jpermanne Exp $
...
function aff_query_auth_empr($field,&$check_scripts,$script="") {
	...
	if ($options['MULTIPLE'][0]['value']=="yes") {
		...
		function fonction_selecteur_".$field["NAME"]."() {
			...
			name_id = name;
			openPopUp('./select.php?what=$what&caller=$caller&p1='+name_id+'&p2=f_'+name_id+'&perso_id=".$field["ID"]."&custom_prefixe=".$_custom_prefixe_."&perso_name=".$field['NAME']."&id_thes_unique=".urlencode($id_thes_unique)."', 'select_author2', 400, 400, -2, -2, 'toolbar=no, dependent=yes, resizable=yes, scrollbars=yes');
		}
...
function aff_query_auth_empr_search($field,&$check_scripts,$varname) {
	...
	$ret.="<input type='button' class='bouton' value='...' title='".htmlentities($msg['title_select_from_list'],ENT_QUOTES,$charset)."' onclick=\"openPopUp('./select.php?what=$what&caller=$caller".$element."&p1=".$field["NAME"]."_0&p2=f_".$field["NAME"]."_0&perso_id=".$field["ID"]."&custom_prefixe=".$_custom_prefixe_."&dyn=$val_dyn&perso_name=".$field['NAME']
...
function aff_list_empr($field,&$check_scripts,$script="") {
	...
	if ($options["AUTORITE"][0]["value"]!="yes") {
		...
		$ret.="<input type='hidden' value='$n' name='n_".$field["NAME"]."' id='n_".$field["NAME"]."' />\n<div id='div_".$field["NAME"]."'>
			<input type='button' class='bouton' value='...' onclick=\"openPopUp('./select.php?what=perso&caller=$caller&p1=".$field["NAME"]."_0&p2=f_".$field["NAME"]."_0&perso_id=".$field["ID"]."&custom_prefixe=".$_custom_prefixe_."&dyn=$val_dyn&perso_name=".$field['NAME']."', 'select_perso_".$field["ID"]."', 700, 500, -2, -2, 'toolbar=no, dependent=yes, resizable=yes, scrollbars=yes')\" /> ";
		$readonly='';
		if($options['MULTIPLE'][0]['value']=="yes") {
	...
	if ($options["AUTORITE"][0]["value"]!="yes") {
		...
		$ret="<script>
			function fonction_selecteur_".$varname."() {
				name=this.getAttribute('id').substring(4);
				name_id = name;
				openPopUp('./select.php?what=perso&caller=search_form&p1='+name_id+'&p2=f_'+name_id+'&perso_id=".$field["ID"]."&custom_prefixe=".$_custom_prefixe_."&dyn=1&perso_name=".$varname."', 'select_author2', 400, 400, -2, -2, 'toolbar=no, dependent=yes, resizable=yes, scrollbars=yes');
			}
		...
		$ret.="<input type='button' class='bouton' value='...' onclick=\"openPopUp('./select.php?what=perso&caller=search_form&p1=".$varname."&p2=f_".$varname."&perso_id=".$field["ID"]."&custom_prefixe=".$_custom_prefixe_."&dyn=1&perso_name=".$varname."', 'select_perso_".$field["ID"]."', 700, 500, -2, -2, 'toolbar=no, dependent=yes, resizable=yes, scrollbars=yes')\" />";
...
function val_url_empr($field,$value) {
	...
	for ($i=0;$i<count($values);$i++){
		...
		$without .= $lib;
		$details[] = array('url' => $val[0], 'label' => $lib);
	}
...
function get_block_dnd($field_type, $field_name, $count, $html, $avalues='') {
	...
	return "
		<div id='customfield_".$field_type."_".$field_name."_".$count."'  class='row' dragtype='customfield_".$field_type."_".$field_name."' draggable='yes' recept='yes' recepttype='customfield_".$field_type."_".$field_name."' handler='customfield_".$field_type."_".$field_name."_".$count."_handle'
			dragicon=\"".$base_path."/images/icone_drag_notice.png\" dragtext=\"".htmlentities($avalues,ENT_QUOTES,$charset)."\" downlight=\"customfield_downlight\" highlight=\"customfield_highlight\"
			order='".$count."' style='' >
			<span id=\"customfield_".$field_type."_".$field_name."_".$count."_handle\" style=\"float:left; padding-right : 7px\"><img src=\"".$base_path."/images/sort.png\" style='width:12px; vertical-align:middle' /></span>
			".$html."
		</div>";
}
...
function get_custom_dnd_on_add() {
	...
	<script type='text/javascript'>
		function get_custom_dnd_on_add(node_id, field_name, count) {
			...
			dnd_div.setAttribute('dragicon', '".$base_path."/images/icone_drag_notice.png');
			...
			sort_icon.setAttribute('src', '".$base_path."/images/sort.png');
