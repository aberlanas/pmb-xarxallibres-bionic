...
// +-------------------------------------------------+
// $Id: func_pro.inc.php,v 1.45.2.1 2017-11-14 14:31:43 dgoron Exp $
...
function dsi_list_bannettes_info($form_cb="", $id_bannette=0, $id_classement="") {
	...
	if($nbr_lignes) {
		...
		while(($ban=pmb_mysql_fetch_object($res))) {
			...
			$td_javascript.=$link_pagination;
			$td_javascript.="';\" ";
			if($equ_trouvees == 0) $bann_list .= "<td style='vertical-align:top' $td_javascript><ul>".$msg['dsi_ban_no_equ']."</ul></td>";
				else $bann_list .= "<td style='vertical-align:top' $td_javascript><ul>$equations</ul></td>";
			...
			$td_javascript.=$link_pagination;
			$td_javascript.="';\" ";
			if ($bann->num_panier) $aff_bann_fills_basket = "&nbsp;&nbsp;<img src='./images/basket_small_20x20.gif' border='0' class='center' />";
				else $aff_bann_fills_basket = "";
			$bann_list .= "<td style='vertical-align:top' $td_javascript>".$bann->nb_abonnes.$aff_bann_fills_basket."</td>";
