...
// +-------------------------------------------------+
// $Id: cart.inc.php,v 1.10 2017-06-16 09:33:40 mbertin Exp $
...
switch ($action) {
	case 'new_cart':
		$myCart= new caddie();
		$form_action = "./circ.php?categ=resa_planning&resa_action=search_resa&mode=3&unq=".md5(microtime())."&id_empr=$id_empr&groupID=$groupID&action=valid_new_cart&item=".$item;
		$form_cancel = "./circ.php?categ=resa_planning&resa_action=search_resa&mode=3&unq=".md5(microtime())."&id_empr=$id_empr&groupID=$groupID&action=&item=".$item;
		print $myCart->get_form($form_action, $form_cancel);
		break;
	case 'del_cart':
		$myCart= new caddie($idcaddie);
		$myCart->delete();
		aff_paniers($idcaddie, "NOTI", "./circ.php?categ=resa_planning&resa_action=search_resa&mode=3&unq=".md5(microtime())."&id_empr=$id_empr&groupID=$groupID", "add_item", $msg['caddie_select_afficher'], "NOTI", 0, 1, 1);
		break;
	case 'del_item':
		$myCart= new caddie($idcaddie);
	...
	default:
		if($idcaddie) {
			...
			print pmb_bidi("<div class=\"row\"><b>Panier&nbsp;: ".$myCart->name.' ('.$myCart->type.')</b></div>');
			aff_cart_notices($myCart->get_cart(), $myCart->type, $idcaddie);
		} else aff_paniers($idcaddie, "NOTI", "./circ.php?categ=resa_planning&resa_action=search_resa&mode=3&unq=".md5(microtime())."&id_empr=$id_empr&groupID=$groupID", "add_item", $msg['caddie_select_afficher'], "NOTI", 0, 1, 1);
}
