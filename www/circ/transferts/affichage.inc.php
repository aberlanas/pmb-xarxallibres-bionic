...
// +-------------------------------------------------+
// $Id: affichage.inc.php,v 1.15 2017-01-04 08:57:11 ngantier Exp $
...
function affiche_liste_departs($typeListe, $page, $rqt_select, $rqt_base, $nb_lignes, $html_global, $html_tableau, $html_tableau_ligne, $html_boutons, $html_pas_de_resultats, $lien_edition="", $autres_filtres="",$url_localisation='') {
	...
	if ($nbRes>0) {
		...
		while ($values=pmb_mysql_fetch_array($res_rqt)) {
			...
			$tmpLigne = str_replace("!!transfert_ask_user!!",aff_user($values['transfert_ask_user_num']), $tmpLigne);	
			//affichage de l'utilisateur qui fait l'envoi
			$tmpLigne = str_replace("!!transfert_send_user!!",aff_user($values['transfert_send_user_num']), $tmpLigne);
