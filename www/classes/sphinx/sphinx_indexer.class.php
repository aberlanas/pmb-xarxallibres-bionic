...
// +-------------------------------------------------+
// $Id: sphinx_indexer.class.php,v 1.6.2.1 2018-01-09 13:58:14 vtouchard Exp $
	...
	protected function getSpecificsFiltersValues($id, $inserts){
		...
		foreach($filters as $name => $values){
			...
			for($i=0 ; $i<count($values) ; $i++){
				$assertions[] = $name.'_'.$values[$i];
			}
		}
		foreach($inserts as $index => $fields){
			$fields['specifics_filters'] = implode($this->separator, $assertions);
			$inserts[$index] = $fields;
		}
		return $inserts;
	}
