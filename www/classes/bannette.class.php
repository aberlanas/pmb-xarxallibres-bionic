...
// +-------------------------------------------------+
// $Id: bannette.class.php,v 1.175.2.9 2018-01-31 10:57:06 jpermanne Exp $
...
class bannette {
	...
	public function set_properties_from_form() {
		...
		global $id_empr;
		global $bannette_auto, $periodicite, $diffusion_email, $statut_not_account, $nb_notices_diff;
	...
	public function get_display_document() {
		...
		$this->build_lists(0);
		$document = "<html><head><META HTTP-EQUIV='CONTENT-TYPE' CONTENT='text/html; charset=".$charset."'>".$this->get_css_style()."</head><body>";
	...
	protected function get_display_mail() {
		...
		$this->set_records_globals();
		$this->build_lists(1);
		$display = "<html><head><META HTTP-EQUIV='CONTENT-TYPE' CONTENT='text/html; charset=".$charset."'>".$this->get_css_style()."</head><body>";
		if($this->bannette_aff_notice_number){
			$display .= "<span class=\"dsi_hide_for_emails\"><hr />!!dsi_diff_n_notices!!<hr /></span>";
		}
		if(($this->bannette_tpl_num) || (($this->proprio_bannette)&&($this->get_private_bannette_tpl()))){
	...
	protected function get_formatted_mail($empr) {
		...
		$texte = str_replace("!!dsi_diff_n_notices!!", sprintf($lang_messages["dsi_diff_n_notices"],$nb_envoyees,$this->nb_notices), $texte);
			
		$dates = time();
		$code=md5($opac_connexion_phrase.$empr->empr_login.$dates);
		$texte = str_replace('!!empr_connect!!',"<a href='".$opac_url_base."empr.php?code=!!code!!&emprlogin=!!login!!&date_conex=!!date_conex!!'>".$lang_messages["selvars_empr_auth_opac"]."</a>",$texte);
		...
		while ($list_empr=pmb_mysql_fetch_object($res_list_empr)) {
			...
			if ($emaildest) {
				if ((!$empr->allow_dsi && !$empr->proprio_bannette) || (!$empr->allow_dsi_priv && $empr->proprio_bannette)) {
					...
					$res_envoi=@mailpmb($empr->empr_prenom." ".$empr->empr_nom, $emaildest,$this->comment_public,$this->get_formatted_mail($empr),$PMBuserprenom." ".$PMBusernom, $PMBuseremail, $headers, "", "", 0, $pieces_jointes);
			...
			while(list($cle, $object) = each($liste)) {
				...
				if (pmb_mysql_num_rows($fetch)) {
					...
					if($notice->niveau_biblio != 's' && $notice->niveau_biblio != 'a') {
						...
						if (!$no_del) 
							$lien_suppr_cart = "<a href='$url_base&suite=suppr_notice&num_notice=$notice->notice_id&id_bannette=$this->id_bannette'><img src='./images/basket_empty_20x20.gif' alt='basket' title=\"".$msg['caddie_icone_suppr_elt']."\" /></a> ";
						else 
							$lien_suppr_cart = "" ;
	...
	protected function get_record_isbd($n) {
		...
		$isbd .= "<a href='".$opac_url_base."index.php?database=".DATA_BASE."&lvl=notice_display&id=".$n->notice_id."&code=!!code!!&emprlogin=!!login!!&date_conex=!!date_conex!!'><b>".$notice_display->header."</b></a><br /><br />\r\n";
