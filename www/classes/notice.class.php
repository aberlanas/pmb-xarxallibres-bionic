...
// +-------------------------------------------------+
// $Id: notice.class.php,v 1.269.2.5 2017-11-17 14:58:43 jpermanne Exp $
...
// 			return '';
				...
				for ($i=0; $i<count($perso_["FIELDS"]); $i++) {
					...
					if (strncmp($p["NAME"], $c1,6)== 0) {
						$perso1.="<div id='move_".$p["NAME"]."' movable='yes' title=\"".htmlentities($msg["notice_convo_type_id"],ENT_QUOTES, $charset)."\">
										<div class='row'><label for='".$p["NAME"]."' class='etiquette'>".htmlentities($msg["notice_convo_type_id"],ENT_QUOTES, $charset)."</label>".$p["COMMENT_DISPLAY"]."</div>
										<div class='row'>".$p["AFF"]."</div>
									 </div>";
					}else{
						if (strncmp($p["NAME"], $c2,6)== 0) {
							$perso1.="<div id='move_".$p["NAME"]."' movable='yes' title=\"".htmlentities($msg["notice_convo_langue"],ENT_QUOTES, $charset)."\">
									<div class='row'><label for='".$p["NAME"]."' class='etiquette'>".htmlentities($msg["notice_convo_langue"],ENT_QUOTES, $charset)."</label>".$p["COMMENT_DISPLAY"]."</div>
									<div class='row'>".$p["AFF"]."</div>
									</div>";
							...
							if (strncmp($p["NAME"], $c3,6)== 0) {
								$perso1.="<div id='move_".$p["NAME"]."' movable='yes' title=\"".htmlentities($msg["notice_convo_authorship"],ENT_QUOTES, $charset)."\">
										<div class='row'><label for='".$p["NAME"]."' class='etiquette'>".htmlentities($msg["notice_convo_authorship"],ENT_QUOTES, $charset)."</label>".$p["COMMENT_DISPLAY"]."</div>
										<div class='row'>".$p["AFF"]."</div>
										</div>";
							}else{
								if (strncmp($p["NAME"], $c4,6)== 0) {
									$perso1.="<div id='move_".$p["NAME"]."' movable='yes' title=\"".htmlentities($msg["notice_convo_literary_work"],ENT_QUOTES, $charset)."\">
										<div class='row'><label for='".$p["NAME"]."' class='etiquette'>".htmlentities($msg["notice_convo_literary_work"],ENT_QUOTES, $charset)."</label>".$p["COMMENT_DISPLAY"]."</div>
										<div class='row'>".$p["AFF"]."</div>
										</div>";
								}else{
									...
									if (strncmp($p["NAME"], $c5,6)== 0) {
										...
										<div class='row'><label for='".$p["NAME"]."' class='etiquette'>".htmlentities($msg["notice_convo_price"],ENT_QUOTES, $charset)."</label>".$p["COMMENT_DISPLAY"]."</div>
										<div class='row'>".$p["AFF"]."</div>
										</div>";
									}else{	
										if (strncmp($p["NAME"], $c6,6)== 0) {
											$perso1.="<div id='move_".$p["NAME"]."' movable='yes' title=\"".htmlentities($msg["notice_convo_location"],ENT_QUOTES, $charset)."\">
											<div class='row'><label for='".$p["NAME"]."' class='etiquette'>".htmlentities($msg["notice_convo_location"],ENT_QUOTES, $charset)."</label>".$p["COMMENT_DISPLAY"]."</div>
											<div class='row'>".$p["AFF"]."</div>
											</div>";
										}		
									}
		...
		//	if(!$user_lang) $user_lang="fr_FR";
			...
			$form_notice = str_replace('!!tab10!!', thumbnail::get_js_function_chklnk_tpl().$ptab[10], $form_notice);				
...
// 				if ($this->notice_mere_id || $this->duplicate_from_id) $link_annul = "<input type='button' class='bouton' value=\"{$msg[76]}\" onClick=\"unload_off();history.go(-1);\" />";
		...
		public function replace($by,$supp_notice=true) {
			...
			vedette_composee::replace(10, $this->id, $by);
			// Mise à jour des vedettes composées contenant cette notice
			vedette_composee::update_vedettes_built_with_element($by, "records");
		...
		public static function del_notice ($id) {
			...
			global $dbh,$class_path,$pmb_synchro_rdf;
		...
		public static function delete_vedette_links($id) {	
			...
			if (pmb_mysql_num_rows($res_responsability)) {
				while($r=pmb_mysql_fetch_object($res_responsability)){
					...
					switch($type_aut){
						case 0:
							$id_vedette=vedette_link::delete_vedette_link_from_object(new vedette_composee(0,'notice_authors'), $object_id, TYPE_NOTICE_RESPONSABILITY_PRINCIPAL);
							break;
						case 1:
							$id_vedette=vedette_link::delete_vedette_link_from_object(new vedette_composee(0,'notice_authors'), $object_id,TYPE_NOTICE_RESPONSABILITY_AUTRE);
							break;
						case 2:
							$id_vedette=vedette_link::delete_vedette_link_from_object(new vedette_composee(0,'notice_authors'), $object_id,TYPE_NOTICE_RESPONSABILITY_SECONDAIRE);
							break;
					}
			...
			foreach ($childs as $childs_relations) {
				foreach ($childs_relations as $child) {
					if(!in_array($child->get_linked_notice(),$liste)) {
						...
						$tab_tmp=notice::get_list_child($child->get_linked_notice(),$liste);
		...
		public static function majNotices($notice){
			...
			if($notice){
				...
				if(pmb_mysql_num_rows($query)) {
					...
					// Mise à jour des vedettes composées contenant cette notice
					vedette_composee::update_vedettes_built_with_element($notice, "records");
				}
			}		
		}
		...
		public function get_concepts_ids(){
			if (!isset($this->concepts_ids)) {
				...
				$vedette_composee_found = vedette_composee::get_vedettes_built_with_element($this->id, 'records');
