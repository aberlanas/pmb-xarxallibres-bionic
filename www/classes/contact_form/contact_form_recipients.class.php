...
// +-------------------------------------------------+
// $Id: contact_form_recipients.class.php,v 1.2 2017-04-19 10:14:15 dgoron Exp $
...
class contact_form_recipients {
	...
	protected function _get_recipients_lines($id) {
		...
		<tr class='odd'>
			...
			<td><input type='text' class='saisie-30em' id='recipient_".$this->mode."_".$id."_name' name='recipients[".$this->mode."][".$id."][name]' value='".(isset($this->recipients[$this->mode][$id]['name']) ? $this->recipients[$this->mode][$id]['name'] : '')."' /></td>	
		</tr>
		<tr class='even'>
			<td><i>".htmlentities($msg['admin_opac_contact_form_recipient_email'], ENT_QUOTES, $charset)."</i></td>
			<td><input type='text' class='saisie-30em' id='recipient_".$this->mode."_".$id."_email' name='recipients[".$this->mode."][".$id."][email]' value='".(isset($this->recipients[$this->mode][$id]['email']) ? $this->recipients[$this->mode][$id]['email'] : '')."' /></td>	
		</tr>
		<tr class='odd'>
			<td><i>".htmlentities($msg['admin_opac_contact_form_recipient_copy_email'], ENT_QUOTES, $charset)."</i></td>
			<td><input type='text' class='saisie-30em' id='recipient_".$this->mode."_".$id."_copy_email' name='recipients[".$this->mode."][".$id."][copy_email]' value='".(isset($this->recipients[$this->mode][$id]['copy_email']) ? $this->recipients[$this->mode][$id]['copy_email'] : '')."' /></td>	
		</tr>
		<tr class='even'>
			<td><i>".htmlentities($msg['admin_opac_contact_form_recipient_transmitter_email'], ENT_QUOTES, $charset)."</i></td>
			<td><input type='text' class='saisie-30em' id='recipient_".$this->mode."_".$id."_transmitter_email' name='recipients[".$this->mode."][".$id."][transmitter_email]' value='".(isset($this->recipients[$this->mode][$id]['transmitter_email']) ? $this->recipients[$this->mode][$id]['transmitter_email'] : '')."' /></td>	
		</tr>	
		";
	}
			...
			foreach ($this->recipients['by_persons'] as $id=>$person) {
				$display .= "
					<tr id='recipient_".$this->mode."_".$id."'>
						<td colspan='2'>
							<table>
								<tr><th colspan='2'>".($this->recipients[$this->mode][$id]['name'] ? $this->recipients[$this->mode][$id]['name'] : htmlentities($msg['admin_opac_contact_form_recipient_without_name'], ENT_QUOTES, $charset))."</th></tr>";
