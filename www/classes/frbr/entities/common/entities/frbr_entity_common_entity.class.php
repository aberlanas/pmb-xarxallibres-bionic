...
// +-------------------------------------------------+
// $Id: frbr_entity_common_entity.class.php,v 1.23.2.2 2017-12-06 08:49:22 tsamson Exp $
	...
	public function get_form($ajax= true) {
		...
		$form .= "
			<form class='form-".$current_module."' id='".$this->class_name."_form' name='".$this->class_name."_form'  method='post' action=\"".htmlentities($action, ENT_QUOTES, $charset)."\" onsubmit='return false;'>
				<h3>".$this->format_text(($this->id ? sprintf($this->msg['frbr_entity_common_entity_'.$this->type.'_edit'],$this->informations['name']." : ".$this->name) : $this->msg['frbr_entity_common_entity_'.$this->type.'_add']))."</h3>
