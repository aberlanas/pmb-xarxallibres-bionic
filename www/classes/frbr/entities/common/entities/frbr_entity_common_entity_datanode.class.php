...
// +-------------------------------------------------+
// $Id: frbr_entity_common_entity_datanode.class.php,v 1.31 2017-06-02 09:52:43 tsamson Exp $
...
class frbr_entity_common_entity_datanode extends frbr_entity_common_entity {
	...
	protected function get_filters_list_form(){
		...
		$form.="
 			...
 			<div class='row'>
 				<div class='colonne3'>
 					<label>".$this->format_text($this->msg['frbr_entity_common_entity_datanode_filter_choice'])."
 				</div>
	...
	protected function get_sorting_list_form(){
		...
		$form.="
 			...
 			<div class='row'>
 				<div class='colonne3'>
 					<label>".$this->format_text($this->msg['frbr_entity_common_entity_datanode_sort_choice'])."
 				</div>
	...
	public function get_children_filter() {
		...
		$optgroup = '';
		if(count($children)){
			$optgroup = "<optgroup label='".htmlentities($msg["frbr_filter_linked_datanodes"],ENT_QUOTES,$charset)."' class='erreur'>\n
						";
			foreach ($children as $id => $name) {
				$optgroup .= "<option value='".$id."'>".htmlentities($name,ENT_QUOTES,$charset)."</option>";
			}
			$optgroup .= "</optgroup>";
		}
		return $optgroup;
	...
	public function get_datanode_datas($data, $limit = false) {
		...
		$datasource = new $datanode_datasource_class_name($this->get_datasource()['id']);
		...
		if (isset($this->get_sort()['id']) && $this->get_sort()['id']!= 0) {
			...
			$datasource->set_sort($sort);
		}
		if (count($data)) {
			$data = $datasource->get_datas($data);
		}
		return $data;
	}
