...
// +-------------------------------------------------+
// $Id: frbr_pages.class.php,v 1.6 2017-07-11 14:35:50 tsamson Exp $
...
class frbr_pages {
	...
	public function get_display_content_list($entity_name='') {
		...
		foreach($this->pages[$entity_name] as $page) {
			...
			$display .= '<td '.$td_javascript.' '.$td_css_style.' align="center">'.($page->get_parameter_value('records_list') ? 'X' : '').'</td>';
			$display .= '<td '.$td_javascript.' '.$td_css_style.' align="center">'.($page->get_parameter_value('facettes_list') ? 'X' : '').'</td>';
			$display .= '<td '.$td_javascript.' '.$td_css_style.' align="center">'.($page->get_parameter_value('isbd') ? 'X' : '').'</td>';
			$display .= '<td '.$td_javascript.' '.$td_css_style.' align="center">'.$page->get_parameter_value('template_directory').'</td>';
			$display .= '<td '.$td_javascript.' '.$td_css_style.' align="center">'.$page->get_parameter_value('record_template_directory').'</td>';
			$display .= '<td align="center"><input type="button" class="bouton" value="'.$msg['frbr_page_tree_build'].'" onclick=\'document.location="'.$base_path.'/cms.php?categ=frbr_pages&sub=build&num_page='.$page->get_id().'&num_parent=0"\' /></td>';
			$display .= '</tr>';
		}
		return $display;
	}
