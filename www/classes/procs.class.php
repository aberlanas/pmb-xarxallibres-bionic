...
// +-------------------------------------------------+
// $Id: procs.class.php,v 1.7.2.2 2017-12-06 10:28:04 jpermanne Exp $
...
class procs {
		...
		while($row = pmb_mysql_fetch_object($result)) {
			...
			if ((static::$module=='admin' && ($PMBuserid==1 || array_search ($PMBuserid, $rqt_autorisation)!==FALSE))
				...
				if(static::$module=='edit') {
					...
					$action = "onmousedown=\"document.location='./admin.php?categ=proc&sub=proc&action=modif&id=".$row->idproc."';\"";
					$buf_contenu.="
						<td width='10'>
						<input class='bouton' type='button' value=' $msg[708] ' onClick=\"document.location='./admin.php?categ=proc&sub=proc&action=execute&id=".$row->idproc."'\" />
						</td>
						<td $action>
							<strong>".$row->name."</strong><br />
							<small>".$row->comment."&nbsp;</small>
	...
	public static function get_proc_form($id=0) {
		...
		if($id) {
			...
			if(pmb_mysql_num_rows($result)) {
				...
				$form = str_replace('!!action!!', $base_path."/".static::$module.".php?categ=proc&sub=proc&action=modif&id=".$id, $form);
			...
			$autorisations[$PMBuserid][0]=1;
			
			$form = str_replace('!!form_title!!', $msg[704], $form);
			$form = str_replace('!!action!!', $base_path."/".static::$module.".php?categ=proc&sub=proc&action=add", $form);
		...
		$form .= confirmation_delete("./".static::$module.".php?categ=proc&sub=proc&action=del&id=");
		return $form;
	}
	...
	public static function run_form($id) {
		...
		if (preg_match_all("|!!(.*)!!|U",$hp->proc->requete,$query_parameters))
			$hp->gen_form(static::$module.".php?categ=proc&sub=proc&action=final&id=".$id."&force_exec=".$force_exec);
		else echo "<script>document.location='".static::$module.".php?categ=proc&sub=proc&action=final&id=".$id."&force_exec=".$force_exec."'</script>";
	}
	...
	public static function run_query($query_code) {
		...
		if((static::$module == 'admin') && $do_reindexation){
			print "<font color='#ff0000'><h2>".$msg['admin_proc_reindex']."</h2><br/>";
		}
	...
	public static function final_execute() {
		...
		if ($is_external) {
			...
			$urlbase = "./".static::$module.".php?categ=proc&sub=proc&action=final&id=$id";
		}
		if($nbr_lignes) {
