...
// +-------------------------------------------------+
// $Id: authperso.class.php,v 1.66.2.3 2017-12-06 14:56:45 dgoron Exp $
...
class authperso {
	...
	public function get_view($id){
		...
		if(($r=pmb_mysql_fetch_object($res))) {
			...
			if($r->authperso_view_script){
				...
				$h2o = new H2o($base_path.'/temp/'.LOCATION.'_authperso_view_'.$r->authperso_authority_authperso_num);
		...
		if(!$form_only) {
			...
			if($sub == 'authperso_last') { // les derniers créés
				...
				if ( pmb_mysql_num_rows($res)) {
 				...
 				$authperso_searcher = searcher_factory::get_searcher('authperso', '', $user_input);
				$nbr_lignes = $authperso_searcher->get_nb_results();
				$sorted_authperso = $authperso_searcher->get_sorted_result('default', $debut, $nb_per_page_gestion);
			}
		}
	...
	public static function get_isbd($id){
		...
		if(($r=pmb_mysql_fetch_object($res))) {			
			...
			if($r->authperso_isbd_script){
				...
				$h2o = new H2o($base_path.'/temp/'.LOCATION.'_authperso_isbd_'.$r->authperso_authority_authperso_num);
	...
	public function get_form($id, $duplicate = false) {
		...
		$id+=0;
		$p_perso=new custom_parametres_perso("authperso","authperso",$this->id,"./autorites.php?categ=authperso&sub=update&id_authperso=".$this->id);
	...
	public function update_from_form($id=0) {
		...
		vedette_composee::update_vedettes_built_with_element($id,$this->id+1000);
	...
	public function delete($id) {
		...
		$attached_vedettes = vedette_composee::get_vedettes_built_with_element($id, $this->id+1000);
		...
		// vedette_composee::delete_element_and_update_vedettes_built_with_element($id,$this->id+1000);
