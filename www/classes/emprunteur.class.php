...
// +-------------------------------------------------+
// $Id: emprunteur.class.php,v 1.177.2.4 2017-11-14 14:31:43 dgoron Exp $
...
class emprunteur {
	...
	public function do_fiche() {
		...
		if ($this->mail) {
			$mail_click = "onclick=\"if (confirm('".$msg["mail_pret_confirm"]."')) { openPopUp('./pdf.php?pdfdoc=mail_liste_pret&id_empr=".$this->id."', 'print_PDF', 600, 500, -2, -2, '');} return(false) \"";
		...
		if ($this->retard>=1) {
			...
			$bouton_lettre_retard = "<font color='RED'>".$msg['retard']." (".$this->retard.")</font>&nbsp;<input type=\"button\" class=\"bouton\" value=\"".$msg["lettre_retard"]."\" ".$imprime_click.">";
...
// 				$vdr=max($vdr,$valeur['org_ret_date']);
		...
		if (count($bannette_abon->tableau_gerer_bannette("PUB")) || count($bannette_abon->tableau_gerer_bannette("PRI"))) {
			$dsi_script .="
				<script type='text/javascript'>			
					function save_bannette_abon(){
						var elts=document.forms['bannette_abonn_PUB'].getElementsByClassName('checkbox_bannette_abon');
						var params='';
						for(var i=0; i<elts.length; i++){
							if(elts[i].checked==true){
								params+= 'bannette_abon['+elts[i].getAttribute('id_bannette')+']=1&';
							}
						}
					...
					function delete_bannette_abon(){
						var elts=document.forms['bannette_abonn_PRI'].getElementsByClassName('checkbox_bannette_abon');
						var params='';
						for(var i=0; i<elts.length; i++){
							if(elts[i].checked==true){
								params+= 'bannette_abon['+elts[i].getAttribute('id_bannette')+']=1&';
				...
				<div id='empr_dsi".$this->id."' class='notice-parent'><h3>
					<img src='./images/plus.gif' class='img_plus' name='imEx' id='empr_dsi".$this->id."Img' title='".$msg['plus_detail']."' border='0' onClick=\"expandBase('empr_dsi".$this->id."', true); bannette_expand_form(); return false;\" hspace='3'>
	...
	public function get_bannette_form() {
		...
		$bannette_abon_instance = new bannette_abon(0, $this->id);
		$bannette_pub = $bannette_abon_instance->gerer_abon_bannette("PUB", "./dsi.php?categ=bannettes&sub=pro&id_bannette=!!id_bannette!!&suite=acces");
		if ($bannette_pub)	$bannette_pub = "<h3><span>" . $msg['dsi_bannette_gerer_pub'] . "</span></h3>\n" . $bannette_pub;
		
		$bannette_pri = $bannette_abon_instance->gerer_abon_bannette("PRI", "./dsi.php?categ=bannettes&sub=abo&id_bannette=!!id_bannette!!&suite=modif");
		if ($bannette_pri) $bannette_pri = "<h3><span>" . $msg['dsi_bannette_gerer_priv'] . "</span></h3>\n" . $bannette_pri;
		
		return $bannette_pub . $bannette_pri;
	}
