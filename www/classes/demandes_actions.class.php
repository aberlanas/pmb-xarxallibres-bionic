...
// +-------------------------------------------------+
// $Id: demandes_actions.class.php,v 1.41.2.1 2017-11-17 09:41:43 wlair Exp $
			...
			foreach($actions as $id_action=>$action){
				...
				$liste .= 	"</td>
					...
					<td><span dynamics='demandes,progression' dynamics_params='text' id='progression_".$action->id_action."' >
						<img src=\"./images/jauge.png\" height='16px' width=\"".$action->progression_action."%\" title='".$action->progression_action."%' />
						</span>
					</td>
				...
				if(pmb_mysql_num_rows($res_act)){						
					...
					while(($com = pmb_mysql_fetch_object($res_act))){
						...
						"<tr class='$pair_impair' id='act_$com->id_action' $tr_javascript style='cursor: pointer'>
							...
							<td $action><img src=\"./images/jauge.png\" style='height:16px;' width=\"".$com->progression_action."%\" title='".$com->progression_action."%' /></td>
