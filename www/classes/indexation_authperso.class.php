...
// +-------------------------------------------------+
// $Id: indexation_authperso.class.php,v 1.5 2017-07-18 07:46:56 dgoron Exp $
...
class indexation_authperso extends indexation_authority {
	...
	protected function get_tab_field_insert($object_id, $infos, $order_fields, $isbd, $lang = '') {
		...
		return "(".$authority->get_id().", ".AUT_TABLE_AUTHPERSO.", ".$infos["champ"].", ".$infos["ss_champ"].", ".$order_fields.", '".addslashes(trim($isbd))."', '".addslashes(trim($lang))."', ".$infos["pond"].", 0)";
	}
