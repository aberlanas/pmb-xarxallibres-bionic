...
// +-------------------------------------------------+
// $Id: caddie_root_controller.class.php,v 1.15.2.2 2017-09-14 08:44:06 ngantier Exp $
...
// 				include ("./".$module."/caddie/pointage/search_history.inc.php");
		...
		print "<div>
			<table width='100%'><tbody>
				<tr>
					...
					<td align='right'><a href='#' id='close_cart_div' ><img border='0' align='middle' src='images/close.gif'/></a></td>
				</tr>
			</tbody></table></div>";
		if(count($list)) {
			switch ($type_object) {
				...
				case 'EMPR':
					...
					break;
				default:
					print "<h3>".$msg['authorities_carts']."</h3><br />";
					break;
			}
			for ($i=0; $i<count($list); $i++) {
				...
				print "<div id=\"".$type_object."_".$list[$i]["idcaddie"]."\" recept=\"yes\" recepttype=\"caddie\" downlight=\"cart_downlight\" highlight=\"cart_highlight\"><img src='images/basket_20x20.gif'/>&nbsp;<a href=\"".$pannel_cart_link."\">".htmlentities($list[$i]["name"],ENT_QUOTES,$charset)."<span id=\"".$type_object."_nbitem_".$list[$i]["idcaddie"]."\"> (".$list[$i]["nb_item"].")</span></a>".$pannel_cart_see."</div>";
			}
		} else {
			switch ($type_object) {
				case 'EXPL' :
	...
	public static function get_display_list($type='display', $object_type='') {
		...
		if(sizeof($liste)) {
			$display .= "<div class='row'><a href='javascript:expandAll()'><img src='./images/expand_all.gif' id='expandall' border='0'></a>
			<a href='javascript:collapseAll()'><img src='./images/collapse_all.gif' id='collapseall' border='0'></a>".static::$title."</div>";
			$parity=array();
