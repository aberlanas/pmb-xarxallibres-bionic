...
// +-------------------------------------------------+
// $Id: caddie_procs.class.php,v 1.10.2.4 2018-01-12 11:19:59 jpermanne Exp $
...
class caddie_procs extends procs {
		...
		if($result) {
			...
			while($row = pmb_mysql_fetch_object($result)) {
				...
				if (array_search ($PMBuserid, $autorisations)!==FALSE || $PMBuserid == 1) {
					...
					$parity += 1;
					$action=" onmousedown=\"document.location='".$base_path."/".static::$module.".php?categ=caddie&sub=gestion&quoi=procs&action=modif&id=".$row->idproc."';\"";
					$tr_javascript=" onmouseover=\"this.className='surbrillance'\" onmouseout=\"this.className='$pair_impair'\" ";
					$display .= "<tr class='$pair_impair' $tr_javascript style='cursor: pointer'>";
					if ($row->type != "ACTION"){
						$display .= "	<td width='10'><input class='bouton' type='button' value=' ".$msg['procs_options_tester_requete']." ' onClick=\"document.location='".$base_path."/".static::$module.".php?categ=caddie&sub=gestion&quoi=procs&action=execute&id=".$row->idproc."'\" />";
					}else{
						$display .= "	<td width='10' $action>&nbsp;";
					}
					$display .= "</td>
							<td width='80' $action>".$row->type."</td>
							<td $action>
	...
	public static function get_proc_form($id=0) {
		...
		switch (static::$table) {
			case 'empr_caddie_procs':
				...
				$cancel_link = $base_path."/circ.php?categ=caddie&sub=gestion&quoi=procs";
				break;
			case 'authorities_caddie_procs':
				...
				$cancel_link = $base_path."/autorites.php?categ=caddie&sub=gestion&quoi=procs";
				break;
			case 'caddie_procs':
			default:
				$example_code = $msg['cart_ex_selection']." select notice_id as <b>object_id</b>, <b>'NOTI'</b> as object_type from notices where ...<br />
					...
					EXPL / NOTI / BULL";
				$cancel_link = $base_path."/catalog.php?categ=caddie&sub=gestion&quoi=procs";
				break;
		}
		$form = str_replace('!!example_code!!', $example_code, $form);
		$form = str_replace('!!cancel_link!!', $cancel_link, $form);
		$autorisations = array();
		if($id) {
			...
			if(pmb_mysql_num_rows($result)) {
				...
				if ($row->type != "ACTION") {
					$form=str_replace("!!exec_button!!","<input type='button' class='bouton' value=' ".$msg[708]." ' onClick=\"document.location='".$base_path."/".static::$module.".php?categ=caddie&sub=gestion&quoi=procs&action=execute&id=".$id."'\" />&nbsp;",$form);
				} else {
					$form=str_replace("!!exec_button!!","",$form);
				}
				$form = str_replace('!!form_title!!', $msg["procs_modification"], $form);
				$form = str_replace('!!action!!', $base_path."/".static::$module.".php?categ=caddie&sub=gestion&quoi=procs&action=modif&id=".$id, $form);
				...
				$button_delete = "<input type='button' class='bouton' value=' ".$msg['supprimer']." ' onClick=\"javascript:confirmation_delete(".$id.",'".addslashes($row->name)."')\" />";
			...
			$form = str_replace('!!action!!', $base_path."/".static::$module.".php?categ=caddie&sub=gestion&quoi=procs&action=add", $form);
		...
		$form .= confirmation_delete("./".static::$module.".php?categ=caddie&sub=gestion&quoi=procs&action=del&id=");
		return $form;
	}
	...
	public static function run_form($id) {
		...
		if (preg_match_all("|!!(.*)!!|U",$hp->proc->requete,$query_parameters))
			$hp->gen_form(static::$module.".php?categ=caddie&sub=gestion&quoi=procs&action=final&id=$id");
		else echo "<script>document.location='".static::$module.".php?categ=caddie&sub=gestion&quoi=procs&action=final&id=$id'</script>";
	}
	...
	public static function get_form_after_execution($id, $name, $code, $commentaire, $is_external = false) {
		...
		if (!$is_external) {
			$form .= "
					...
					<input type='button' class='bouton' value='$msg[62]'  onClick='document.location=\"./".static::$module.".php?categ=caddie&sub=gestion&quoi=procs&action=modif&id=$id\"' />
					<input type='button' class='bouton' value='$msg[708]' onClick='document.location=\"./".static::$module.".php?categ=caddie&sub=gestion&quoi=procs&action=execute&id=$id\"' />&nbsp;
					</h3>
			...
			<h3>".$msg["remote_procedures_executing"]." $name</h3>
					...
					<input type='button' class='bouton' value='$msg[708]' onClick='document.location=\"./".static::$module.".php?categ=caddie&sub=gestion&quoi=remote_procs&action=execute_remote&id=$id\"' />
	...
	public static function proceed() {
		...
		switch($action) {
			case 'configure':
				...
				$hp->show_config_screen(static::$module.".php?categ=caddie&sub=gestion&quoi=procs&action=update_config",static::$module.".php?categ=caddie&sub=gestion&quoi=procs");
				break;
			case 'update_config':
				$hp=new parameters($id_query,static::$table);
				$hp->update_config(static::$module.".php?categ=caddie&sub=gestion&quoi=procs");
				break;
			case 'final':
			...
			case 'add':
				...
				break;
			case 'import':
				$import_proc_tmpl = str_replace("!!action!!", "./".static::$module.".php?categ=caddie&sub=gestion&quoi=procs&action=importsuite", $import_proc_tmpl);
				print $import_proc_tmpl ;
				break;
			case 'importsuite':
				static::importsuite("./".static::$module.".php?categ=caddie&sub=gestion&quoi=procs&action=modif&id=!!id!!", "./".static::$module.".php?categ=caddie&sub=gestion&quoi=procs&action=import") ;
				break;
			case 'del':
				if($id) {
					static::delete($id);
	...
	public static function final_execute() {
		...
		if ($is_external) {
			...
			$urlbase = "./".static::$module.".php?categ=caddie&sub=gestion&quoi=procs&action=final&id=$id";
		}
		
		if($nbr_lignes) {
			print static::get_form_after_execution($idp, $name, $code, $commentaire, $is_external);
			// récupération du résultat
			static::run_query($code);
		} else {
