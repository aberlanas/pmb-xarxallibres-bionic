...
// +-------------------------------------------------+
// $Id: searcher.class.php,v 1.184.2.3 2018-01-17 13:23:56 jpermanne Exp $
...
// 			$searcher_authorities_authors = new searcher_authorities_authors(stripslashes($author_query));
			$this->t_query=pmb_mysql_query($searcher_authorities_authors->get_full_query());
			$this->nbresults=pmb_mysql_num_rows($this->t_query);
	...
	public function aut_list() {
		...
		if ($this->nbresults) {
				...
				while ($row=pmb_mysql_fetch_object($this->t_query)) {
					...
					$parity += 1;
					$authority = new authority($row->id_authority);
		...
		if ($this->nbresults) {
				...
				if ($this->rec_history) {
					...
					if ($current!==false) {
						...
						echo "<img src='".get_url_icon('basket_small_20x20.gif')."' style='border:0px' class='center' alt=\"".$msg["histo_add_to_cart"]."\" title=\"".$msg["histo_add_to_cart"]."\"></a>&nbsp;";
						$tri_id_info = $_SESSION["tri"] ? "&sort_id=".$_SESSION["tri"] : "";
						echo "<a href='#' onClick=\"openPopUp('./print.php?current_print=$current&action_print=print_prepare".$tri_id_info."','print'); return false;\">";
						echo "<img src='".get_url_icon('print.gif')."' style='border:0px' class='center' alt=\"".$msg["histo_print"]."\" title=\"".$msg["histo_print"]."\"/></a>&nbsp;";
						echo "<a href='#' onClick=\"openPopUp('./download.php?current_download=$current&action_download=download_prepare".$tri_id_info."','download'); return false;\">";
						echo "<img src='".get_url_icon('upload_docnum.gif')."' style='border:0px' class='center' alt=\"".$msg["docnum_download"]."\" title=\"".$msg["docnum_download"]."\"/></a>";
						if ($pmb_allow_external_search) {
							...
							echo "<img src='".get_url_icon('external_search.png')."' style='border:0px' class='center' alt=\"".$msg["connecteurs_external_search_sources"]."\"/></a>";
						}
						...
						if ($this->nbresults<=$pmb_nb_max_tri) {
							...
							echo "<img src='".get_url_icon('orderby_az.gif')."' class='align_middle' hspace=3></a>";
