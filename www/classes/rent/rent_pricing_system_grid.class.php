...
// +-------------------------------------------------+
// $Id: rent_pricing_system_grid.class.php,v 1.2 2016-02-23 17:29:50 dgoron Exp $
...
class rent_pricing_system_grid {
	...
	protected function get_display_line($element) {
		...
		$display = "<tr><td class='pricing_system_grid_header' align='right'>";
		switch($element['type']) {
			case 1 :
		...
		switch($element['type']) {
			...
			case 2 :
				foreach ($this->percents as $percent) {
					$display .= "<td class='pricing_system_grid_content' align='right'>".number_format(($element['price']*$percent)/100,2,'.','')." &euro;</td>";
				}
				break;
			case 3 :
				$display .= "<td class='pricing_system_grid_content' align='right'>".$element['price']." &euro;</td>";
	...
	public function get_display() {
		...
		$display .= "<tr><td colspan='".(count($this->percents)+1)."' class='brd' align='center'><b>".$this->pricing_system->get_label()."</b></td></tr>";
		$display .= "<tr><td width='10%' align='right'><b>".$msg['pricing_system_grid_time_percent']."</b></td>";
		foreach ($this->percents as $percent) {
			$display .= '<td class="pricing_system_grid_header" align="center">'.$percent.'%</td>';
		}
		$display .= "</tr>";
		if(count($this->grid)){
			foreach ($this->grid as $element) {
	...
	public function get_display_in_layer() {
		...
		$display = '<div class="right"><a href="#" onClick="parent.kill_frame(\'frame_notice_preview\');return false;"><img src="'.$base_path.'/images/close.gif" border="0" align="right"></a></div>';
						...
						if($time) {
							...
							if($time > $last_interval_element['time_end']*1) {
								if($element['time_start']*1) {
									$nb_extra = floor(($time-$last_interval_element['time_end']*1)/$element['time_start']*1);
									if($nb_extra) {
