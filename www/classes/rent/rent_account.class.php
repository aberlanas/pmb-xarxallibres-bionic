...
// +-------------------------------------------------+
// $Id: rent_account.class.php,v 1.37 2017-04-19 12:37:02 dgoron Exp $
...
class rent_account {
	...
	public function get_state_invoice() {
		...
		if($this->num_invoice) {
			...
			if($invoice->get_status() == 1) {
				return "<img src='".$base_path."/images/new.gif' alt='".$msg['acquisition_account_state_unvalidated']."' title='".$msg['acquisition_account_state_unvalidated']."' ".$link_edit_invoice." />";	
			} elseif($invoice->get_status() == 2) {
				return "<img src='".$base_path."/images/notice.gif' alt='".$msg['acquisition_account_state_validated']."' title='".$msg['acquisition_account_state_validated']."' ".$link_edit_invoice." />";
			} else {
				return "";
			}
		} else {
			$link_invoices_selector = "onclick=\"account_show_invoices_selector('".$this->id."');\"";
			return "<img src='".$base_path."/images/req_get.gif' alt='".$msg['account_show_invoices_selector_title']."' title='".$msg['account_show_invoices_selector_title']."' ".$link_invoices_selector." />";
		}
	}	
	...
	public function add_account_in_invoice($invoice_id){
		...
		return array(
			...
			'icon' => "<img onclick=\"document.location='./acquisition.php?categ=rent&sub=invoices&action=edit&id_bibli=&id=".$invoice_id."';\" title='".htmlentities($msg['acquisition_invoice_status_new'], ENT_QUOTES, $charset)."' alt='".htmlentities($msg['acquisition_invoice_status_new'], ENT_QUOTES, $charset)."' src='./images/new.gif'>",
		);
	}
