...
// +-------------------------------------------------+
// $Id: rent_requests.class.php,v 1.5 2016-10-10 13:55:57 dgoron Exp $
...
class rent_requests extends rent_accounts {
	...
	public function get_display_content_list() {
		...
		foreach ($this->objects as $account) {
			...
			$display .= '<td '.$td_javascript.' align="center">'.$account->get_id().'</td>';
			...
			$display .= '<td '.$td_javascript.' align="center">'.formatdate($account->get_date()).'</td>';
			$display .= '<td '.$td_javascript.'>'.$account->get_title().'</td>';
			$display .= '<td '.$td_javascript.'>'.$account->get_publisher()->display.'</td>';
			$display .= '<td '.$td_javascript.'>'.$account->get_supplier()->raison_sociale.'</td>';
			$display .= '<td '.$td_javascript.'>'.$account->get_author()->display.'</td>';
			$display .= '<td '.$td_javascript.' align="center">'.$account->get_formatted_event_date().'</td>';
			$display .= '<td '.$td_javascript.' align="center">'.formatdate($account->get_receipt_limit_date()).'</td>';
			$display .= '<td '.$td_javascript.' align="center">'.formatdate($account->get_receipt_effective_date()).'</td>';
			$display .= '<td '.$td_javascript.' align="center">'.formatdate($account->get_return_date()).'</td>';
	...
	public function get_display_header_list() {
		...
		$display = "
		<tr>
			<th width='5px'><input type='checkbox' id='requests_select_all' value='1' onchange='requests_select();' title=\"".$msg['acquisition_accounts_select_unselect']."\" /></th>
