...
// +-------------------------------------------------+
// $Id: rent_invoices.class.php,v 1.21.4.1 2017-12-11 09:23:13 dgoron Exp $
...
class rent_invoices extends rent_root {
	...
	public function get_display_content_list() {
		...
		if(count($this->objects)) {
			...
			foreach ($this->objects as $invoice) {
				...
				$display .= '<td '.$td_javascript.' align="center">'.$invoice->get_id().'</td>';
				$display .= '<td '.$td_javascript.'>'.$invoice->get_user()->prenom.' '.$invoice->get_user()->nom.'</td>';
				$display .= '<td '.$td_javascript.' align="center">'.formatdate($invoice->get_date()).'</td>';
				$accounts = $invoice->get_accounts();
				if(count($accounts)) {
					$display .= '<td '.$td_javascript.'>'.$accounts[0]->get_publisher()->display.'</td>';
					$display .= '<td '.$td_javascript.'>'.$accounts[0]->get_supplier()->raison_sociale.'</td>';
				} else {
					$display .= '<td '.$td_javascript.'></td>';
					$display .= '<td '.$td_javascript.'></td>';
				}
				$display .= '<td '.$td_javascript.' align="center">'.$invoice->get_status_label().'</td>';
				$display .= '<td '.$td_javascript.' align="center">'.formatdate($invoice->get_valid_date()).'</td>';
	...
	public function get_display_header_list() {
		...
		$display = "
		<tr>
			<th width='5px'><input type='checkbox' id='invoices_select_all' value='1' onchange='invoices_select();' title=\"".$msg['acquisition_invoices_select_unselect']."\" /></th>
	...
	public function get_display_content_selector_list($id_account) {
		...
		foreach ($this->objects as $invoice) {
			...
			$display .= '<td align="center">'.$invoice->get_id().'</td>';
			$display .= '<td>'.$invoice->get_user()->prenom.' '.$invoice->get_user()->nom.'</td>';
			$display .= '<td align="center">'.$invoice->get_formatted_date().'</td>';
			$accounts = $invoice->get_accounts();
			if(count($accounts)) {
				$display .= '<td>'.$accounts[0]->get_publisher()->display.'</td>';
				$display .= '<td>'.$accounts[0]->get_supplier()->raison_sociale.'</td>';
			} else {
				$display .= '<td></td>';
				$display .= '<td></td>';
			}
