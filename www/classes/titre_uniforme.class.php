...
// +-------------------------------------------------+
// $Id: titre_uniforme.class.php,v 1.154.2.6 2017-11-21 10:08:52 tsamson Exp $
...
class titre_uniforme {
	...
	public function get_authors() {
		...
		while ($resp_tu=pmb_mysql_fetch_object($res_sql)) {
			...
			$data = array( 
				...
				'fonction_label' => ($resp_tu->responsability_tu_fonction ? $fonction_auteur[$resp_tu->responsability_tu_fonction] : ''),
				'qualif' => $qualif,
	...
	public function gen_oeuvre_expression_form() {
		...
		for($i = 0; $i < $max_oeuvre_expression; $i ++) {
			...
			if (sizeof ( $oeuvre_expressions ) == 0) {
				...
				$tu_object = new authority(0, $oeuvre_expressions[$i]["to_id"], AUT_TABLE_TITRES_UNIFORMES);
	...
	public function gen_oeuvre_expression_from_form() {
		...
		for($i = 0; $i < $max_oeuvre_expression_from; $i ++) {
			...
			if (sizeof ( $oeuvre_expressions_from ) == 0) {
				...
				$tu_object = new authority(0, $oeuvre_expressions_from[$i]["to_id"], AUT_TABLE_TITRES_UNIFORMES);				
		...
		if(!isset($this->oeuvre_events)){
			...
			if ($result && pmb_mysql_num_rows($result)) {
				while ($auth = pmb_mysql_fetch_object($result)) {
					$authority = new authority(0,$auth->oeuvre_event_authperso_authority_num, AUT_TABLE_AUTHPERSO);
	...
	public function gen_other_link_form() {
		...
		for($i = 0; $i < $max_other_link; $i ++) {
			...
			if (sizeof ( $other_links ) == 0) {
				...
				$tu_object = new authority(0, $other_links[$i]["to_id"], AUT_TABLE_TITRES_UNIFORMES);
	...
	public function replace($by,$link_save) {
		...
		vedette_composee::replace(AUT_TABLE_TITRES_UNIFORMES, $this->id, $by);
...
// 		if ($flag_index)
	...
	public function do_isbd() {
		...
		if(count($oeuvre_expressions) ){
			$isbd_oeuvre_expressions.=" .- ".$msg["aut_oeuvre_form_oeuvre_expression"];
			foreach ($oeuvre_expressions as $expression) {
				$oeuvre_to=authorities_collection::get_authority(AUT_TABLE_TITRES_UNIFORMES, $expression['to_id']);
		...
		if(count($other_links)){		
			for($i=0;$i<count($other_links);$i++) {
				$memo_links[$other_links[$i]['type']][]=$i;				
			}
			foreach ($memo_links as $link =>$index_list){
				$isbd_other_links.=" - ".$other_links[$index_list[0]]['type_label'];
				foreach ($index_list as $index){					
					$oeuvre_link=authorities_collection::get_authority(AUT_TABLE_TITRES_UNIFORMES, $other_links[$index]['to_id']);
	...
	public static function get_format_data_structure($antiloop = false) {
		...
		$authority = new authority(0, 0, AUT_TABLE_TITRES_UNIFORMES);
