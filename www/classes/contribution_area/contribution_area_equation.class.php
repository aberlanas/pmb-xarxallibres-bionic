...
// +-------------------------------------------------+
// $Id: contribution_area_equation.class.php,v 1.2 2017-07-12 15:15:00 tsamson Exp $
...
class contribution_area_equation{
	...
	public static function show_list(){
		...
		foreach(static::$equations as $id => $equation){
			...
			<tr  class='".$pair_impair."' style='cursor: pointer' onmouseover=\"this.className='surbrillance'\" onmouseout=\"this.className='".$pair_impair."'\">
				<td onclick='document.location=\"./admin.php?categ=contribution_area&sub=equation&action=edit&id=".$id."\"'>".htmlentities($equation['name'], ENT_QUOTES, $charset)."</td>
				<td onclick='document.location=\"./admin.php?categ=contribution_area&sub=equation&action=edit&id=".$id."\"'>".htmlentities($equation['type_name'], ENT_QUOTES, $charset)."</td>
			</tr>";
			$i++;
		}
		print "
		...
		<div class='row'>
			<input type='button' class='bouton' value='".$msg['admin_contribution_area_add_equation']."' onclick='document.location=\"./admin.php?categ=contribution_area&sub=equation&action=add\"'/>		
		</div>";
	}
	...
	public function add(){
		...
		$form.= $my_search->show_form("./admin.php?categ=contribution_area&sub=equation&action=build&equation_type=".$type."&id=".$this->id,
				"","","./admin.php?categ=contribution_area&sub=equation&action=form&equation_type=".$type."&id=".$this->id);
		print $form;
	...
	public function get_pmb_entities_selector() {
		...
		$on_change='onchange="document.location=\'./admin.php?categ=contribution_area&sub=equation&section=liste&action=build&equation_type=\'+this.value+\'&id='.$this->id.'\'"';
	...
	public function do_form () {
		...
		$action="./admin.php?categ=contribution_area&sub=equation&action=save&id=".$this->id;
	...
	public function make_hidden_search_form() {
		...
		$url = "./admin.php?categ=contribution_area&sub=equation&action=add&id=".$this->id ;
