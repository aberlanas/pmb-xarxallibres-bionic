...
// +-------------------------------------------------+
// $Id: scheduler_tasks.class.php,v 1.1 2017-07-10 15:50:01 dgoron Exp $
...
class scheduler_tasks {
	...
	public function get_display_list () {
		...
		$display .= "<a href='javascript:expand_taches_all()'><img border='0' id='expandall' src='./images/expand_all.gif'></a>
		...
		foreach($this->tasks as $name=>$tasks_type) {
			...
			$display .= "<tr class='$pair_impair' $tr_javascript style='cursor: pointer' title='".htmlentities($tasks_type->get_comment(),ENT_QUOTES,$charset)."' alter='".htmlentities($tasks_type->get_comment(),ENT_QUOTES,$charset)."' id='tr".$tasks_type->get_id()."'><td>".($n_taches?"<img src='images/plus.gif' class='img_plus' onClick='if (event) e=event; else e=window.event; e.cancelBubble=true; if (e.stopPropagation) e.stopPropagation(); show_taches(\"".addslashes($name)."\"); '/>":"&nbsp;")."</td><td>".htmlentities($tasks_type->get_comment(),ENT_QUOTES,$charset)."</td>
