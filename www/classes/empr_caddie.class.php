...
// +-------------------------------------------------+
// $Id: empr_caddie.class.php,v 1.29 2017-07-10 13:44:32 ngantier Exp $
...
class empr_caddie extends caddie_root {
	...
	public function aff_cart_objects ($url_base="./circ.php?categ=caddie&sub=gestion&quoi=panier&idemprcaddie=0", $no_del=false,$rec_history=0, $no_point=false ) {
		...
		if(!$nbr_lignes) {
			$requete = "SELECT count(1) FROM empr_caddie_content where empr_caddie_id='".$this->get_idcaddie()."' ";
		...
		if($nbr_lignes) {
			...
			$requete = "SELECT object_id, flag FROM $from where empr_caddie_id='".$this->get_idcaddie()."' order by $order_by";
		...
		if(!sizeof($liste) || !is_array($liste)) {
			...
			while(list($cle, $object) = each($liste)) {
				...
				if(pmb_mysql_num_rows($fetch)) {
					...
					if (!$no_point) {
						if ($object['flag']) $marque_flag ="<img src='images/depointer.png' id='caddie_".$this->get_idcaddie()."_item_".$empr->id_empr."' title=\"".$msg['caddie_item_depointer']."\" onClick='del_pointage_item(".$this->get_idcaddie().",".$empr->id_empr.");' style='cursor: pointer'/>" ;
						else $marque_flag ="<img src='images/pointer.png' id='caddie_".$this->get_idcaddie()."_item_".$empr->id_empr."' title=\"".$msg['caddie_item_pointer']."\" onClick='add_pointage_item(".$this->get_idcaddie().",".$empr->id_empr.");' style='cursor: pointer'/>" ;
					} else {
