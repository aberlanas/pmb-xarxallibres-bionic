...
// +-------------------------------------------------+
// $Id: searcher_sphinx.class.php,v 1.5.2.3 2017-12-21 16:24:23 vtouchard Exp $
...
class searcher_sphinx {
	...
	public function __construct($user_query){
		...
		$this->sc->SetMatchMode(SPH_MATCH_EXTENDED2);
 		$this->sc->SetSortMode(SPH_SORT_EXTENDED, '@weight DESC');
  		$this->sc->SetRankingMode(SPH_RANK_PROXIMITY_BM25);
	...
	protected function _get_objects_ids(){
		...
		$this->objects_ids = '';
 		...
 		if ($this->sphinx_query == '*' && !$this->get_filter_query()){
		...
		do {
			$this->sc->SetLimits($nb, $this->bypass);
 			$result = $this->sc->Query($query.$this->get_filter_query(),$this->get_search_indexes());
	...
	protected function _build_tmp_table(){
		...
		$query = 'create temporary table '.$this->get_tempo_tablename().'('.$this->id_key.' int,pert int,index using btree('.$this->id_key.')) engine=memory' ;
		...
		if(!isset($this->filter_query)){
			...
			for($i=0 ; $i<count($filters) ; $i++){
				...
				for ($j=0; $j<count($filters[$i]['values']); $j++) {
					$filter_query.= ' '.$filters[$i]['name'].'_'.$filters[$i]['values'][$j];
				}
			}
			if($filter_query){
				$this->filter_query = ' @(specifics_filters) ('.$filter_query.')';
			}else{
				$this->filter_query = '';
			}
		}
