...
// +-------------------------------------------------+
// $Id: scan_request.class.php,v 1.30.2.2 2017-08-09 14:37:22 arenou Exp $
...
*/	
	...
	public function get_selector_piece($id, $type, $comment, $explnum=array()){
		global $charset;
		$id = $id*1;
		return array('id'=>$id, 'comment'=>$comment, 'label'=>(($type=='record')?$this->get_record_display_header($id):$this->get_bulletin_title($id)),  'explnums'=>$explnum);
	}
	...
	public function get_values_from_form(){
		...
		$this->num_location = $scan_request_num_location+0;
	...
	protected function fetch_explnum($record_id, $bulletin_id){
		...
		if($result){
			while($row = pmb_mysql_fetch_object($result)){
				$this->explnums_ids[] = $row->scan_request_explnum_num_explnum;
	...
	public function del_explnum($explnum_id){
		global $gestion_acces_active, $gestion_acces_user_notice;
		global $PMBuserid;

		$right = in_array($explnum_id,$this->explnums_ids);
		if(!$right) return false;

		//Ici, on est certain que le document numérique est liée à cette demande de numérisation (ca limite déjà pas mal)
		$explnum = new explnum($explnum_id);	
		//On vérifie que l'utilisateur à le droits de supprimer ce document quand même...
		...
		if ($gestion_acces_active==1 && $gestion_acces_user_notice==1) {
			$ac= new acces();
			$dom_1= $ac->setDomain(1);
			$right= $dom_1->getRights($PMBuserid,$id_for_rigths,8);
		}
		if($right == 0) return false;
		
		//on est blindé, le document est liée à la demande et l'utilisateur à le droit, on supprime !
		$explnum->delete();
		return true;
	}
}