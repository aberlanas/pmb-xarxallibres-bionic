...
// +-------------------------------------------------+
// $Id: scan_request_priorities.class.php,v 1.4 2017-01-25 16:43:50 dgoron Exp $
...
class scan_request_priorities {
	...
	public function get_list($form_link="./admin.php?categ=scan_request&sub=priorities&action=edit"){
		...
		for($i=0 ; $i<count($this->scan_request_priorities) ; $i++){
			$class = ($i%2 ? "odd":"even");
			$table.= "
			<tr class='".($i%2 ? "odd":"even")."' onclick='document.location=\"".$form_link."&id=".$this->scan_request_priorities[$i]['id']."\"' onmouseover=\"this.className='surbrillance'\" onmouseout=\"this.className='$pair_impair'\">
				<td>
