...
// +-------------------------------------------------+
// $Id: onto_common_datatype_marclist_ui.class.php,v 1.3.2.1 2017-09-14 14:45:24 tsamson Exp $
		...
		if(sizeof($datas)){
			...
			foreach($datas as $key=>$data){
				...
				foreach($marc_list->table as $value => $label){
					$options.= '<option value="'.$value.'" '.($data->get_formated_value() == $value ? 'selected=selected>' : '>').htmlentities($label,ENT_QUOTES,$charset).'</option>';
				}
