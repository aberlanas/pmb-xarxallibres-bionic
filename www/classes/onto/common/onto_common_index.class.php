...
// +-------------------------------------------------+
// $Id: onto_common_index.class.php,v 1.1.2.3 2018-01-02 16:13:21 vtouchard Exp $
									...
									foreach($property_uri as $property => $sub_property){
										...
										if(isset($row->{$var_name})){
											...
											if(!isset($this->sparql_result[$var_name][$row->{$property_name}])){
		 									...
		 									if(!isset($this->sparql_result[$var_name][$row->{$property_name}][$lang])){
		 										$this->sparql_result[$var_name][$row->{$property_name}][$lang] = array();
		 									}
		 									if (!in_array($row->{$var_name},$this->sparql_result[$var_name][$row->{$property_name}][$lang])){
												$this->sparql_result[$var_name][$row->{$property_name}][$lang][] = $row->{$var_name};
		 									} 
										}							
									}
								}
					...
					foreach($this->sparql_result[$column] as $key => $values){
						foreach($values as $key2 => $value){
							if(is_string($value)){
								$language = $key;
								//fields (contenu brut)
								$tab_fields_insert[] = "('".$object_id."','".$infos['champ']."','".$infos['ss_champ']."','".$field_order."','".addslashes($value)."','".$language."','".$infos['pond']."','".(isset($autority_num) ? $autority_num : "" )."')";
