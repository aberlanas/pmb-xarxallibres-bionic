...
// +-------------------------------------------------+
// $Id: onto_skos_concept_item.class.php,v 1.9 2017-06-14 15:29:34 apetithomme Exp $
...
class onto_skos_concept_item extends onto_common_item {
	...
	public function get_form($prefix_url="",$flag="",$action="save") {
		...
		if(!onto_common_uri::is_temp_uri($this->uri)){
			...
			if ($pmb_type_audit) {
				$form=str_replace("!!onto_form_audit!!", '<input type="button" class="bouton" onclick="openPopUp(\'./audit.php?type_obj='.AUDIT_CONCEPT.'&object_id='.$this->get_id().'\', \'audit_popup\', 700, 500, -2, -2, \'scrollbars=yes, toolbar=no, dependent=yes, resizable=yes\')" title="'.$msg['audit_button'] .'" value="'.$msg['audit_button'].'" />', $form);
			} else {
