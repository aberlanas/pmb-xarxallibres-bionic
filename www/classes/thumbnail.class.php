...
// +-------------------------------------------------+
// $Id: thumbnail.class.php,v 1.3.2.2 2017-09-29 14:11:13 apetithomme Exp $
	...
	public static function get_parameter_img_pics_max_size($object_type = 'record') {
		switch ($object_type) {
			case 'authority':
				...
				break;
			default:
				global $parameter_img_pics_max_size;
				return $parameter_img_pics_max_size;
				break;
		}
	}
		...
		if(!isset(static::$image[$code."_".$thumbnail_url])) {
			if ($code || $thumbnail_url) {
				if ($opac_show_book_pics=='1' && ($opac_book_pics_url || $thumbnail_url)) {
					...
					static::$image[$code."_".$thumbnail_url] = "<img class='vignetteimg' src='".static::get_url_image($code, $thumbnail_url)."' title=\"".$title_image_ok."\" align='right' hspace='4' vspace='2' style='max-width : 140px; max-height: 200px;' >";
				} else {
		...
		if (!empty($notice->code) || !empty($notice->thumbnail_url)) {
			if ($pmb_book_pics_show=='1' && ($pmb_book_pics_url || $notice->thumbnail_url)) {
				...
				if ($depliable) {//MB - 22/06/2017: dépliable à 0 ou pas défini, on ne passe jamais ici je pense
					$image = "<img class='img_notice' id='PMBimagecover".$notice->notice_id."' src='".$prefix_url_image."images/vide.png' align='right' hspace='4' vspace='2' isbn='".$code_chiffre."' url_image='".$url_image."' vigurl=\"".$notice->thumbnail_url."\">";
				} else {
					...
					$image = "<img class='img_notice' id='PMBimagecover".$notice->notice_id."' src='".$url_image_ok."' title=\"".$title_image_ok."\" align='right' hspace='4' vspace='2'>";
				}
			} else {
				$image="";
			}
