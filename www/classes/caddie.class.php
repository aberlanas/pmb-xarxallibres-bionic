...
// +-------------------------------------------------+
// $Id: caddie.class.php,v 1.75.2.2 2017-11-15 15:06:26 dgoron Exp $
...
class caddie extends caddie_root {
	...
	public function save_cart() {
		$query = "update caddie set name='".addslashes($this->name)."', comment='".addslashes($this->comment)."', autorisations='".$this->autorisations."', caddie_classement='".addslashes($this->classementGen)."', acces_rapide='".$this->acces_rapide."' where ".static::get_field_name()."='".$this->get_idcaddie()."'";
	...
	static public function show_actions($id_caddie = 0, $type_caddie = 'NOTI') {
		...
		global $pmb_scan_request_activate, $gestion_acces_active;
	...
	public function aff_cart_objects ($url_base="./catalog.php?categ=caddie&sub=gestion&quoi=panier&idcaddie=0", $no_del=false,$rec_history=0, $no_point=false ) {
		...
		if(!$nbr_lignes) {
			$requete = "SELECT count(1) FROM caddie_content where caddie_id='".$this->get_idcaddie()."' ";
			$res = pmb_mysql_query($requete, $dbh);
		...
		if($nbr_lignes) {
			...
			$requete = "SELECT * FROM $from where caddie_id='".$this->get_idcaddie()."' order by $order_by";
					...
					if ($object['content']=="") {
						...
						if(pmb_mysql_num_rows($fetch)) {
							...
							if ($notice->niveau_biblio == 'b') {
								...
								$link_explnum = '';
								if (!$no_point) {
									if ($object['flag']) $marque_flag ="<img src='images/depointer.png' id='caddie_".$this->get_idcaddie()."_item_".$notice->notice_id."' title=\"".$msg['caddie_item_depointer']."\" onClick='del_pointage_item(".$this->get_idcaddie().",".$notice->notice_id.");' style='cursor: pointer'/>" ;
									else $marque_flag ="<img src='images/pointer.png' id='caddie_".$this->get_idcaddie()."_item_".$notice->notice_id."' title=\"".$msg['caddie_item_pointer']."\" onClick='add_pointage_item(".$this->get_idcaddie().",".$notice->notice_id.");' style='cursor: pointer'/>" ;
								} else {
				...
				while(list($cle, $expl) = each($liste)) {
					if (!$expl['content']) {
						if($stuff = get_expl_info($expl['object_id'])) {
							if (!$no_point) {
								if ($expl['flag']) $marque_flag ="<img src='images/depointer.png' id='caddie_".$this->get_idcaddie()."_item_".$stuff->expl_id."' title=\"".$msg['caddie_item_depointer']."\" onClick='del_pointage_item(".$this->get_idcaddie().",".$stuff->expl_id.");' style='cursor: pointer'/>" ;
								else $marque_flag ="<img src='images/pointer.png' id='caddie_".$this->get_idcaddie()."_item_".$stuff->expl_id."' title=\"".$msg['caddie_item_pointer']."\" onClick='add_pointage_item(".$this->get_idcaddie().",".$stuff->expl_id.");' style='cursor: pointer'/>" ;
							} else {
			...
			if ($this->type=="BULL") {
				...
				while(list($cle, $expl) = each($liste)) {
					...
					if($bull_aff = show_bulletinage_info($expl['object_id'], 0 , $show_del, $expl['flag'],1)) {
						...
						if (!$no_point) {
							if ($expl['flag']) $marque_flag ="<img src='images/depointer.png' id='caddie_".$this->get_idcaddie()."_item_".$expl['object_id']."' title=\"".$msg['caddie_item_depointer']."\" onClick='del_pointage_item(".$this->get_idcaddie().",".$expl['object_id'].");' style='cursor: pointer'/>" ;
							else $marque_flag ="<img src='images/pointer.png' id='caddie_".$this->get_idcaddie()."_item_".$expl['object_id']."' title=\"".$msg['caddie_item_pointer']."\" onClick='add_pointage_item(".$this->get_idcaddie().",".$expl['object_id'].");' style='cursor: pointer'/>" ;
						} else {
							if ($expl['flag']) $marque_flag ="<img src='images/tick.gif'/>" ;
							else $marque_flag ="" ;
						}
	...
	public function aff_cart_nb_items() {
		...
		<div id='cart_".$this->get_idcaddie()."_nb_items' name='cart_".$this->get_idcaddie()."_nb_items'>
			...
			<div class='row'>
				<div class='colonne3' align='right'>".$msg['caddie_contient_dont_fonds']."</div>
				<div class='colonne3' align='center'><label class='etiquette' id='nb_item_base'>".$this->nb_item_base."</label></div>
				<div class='colonne_suite' align='center'><label id='nb_item_base_pointe'>".$this->nb_item_base_pointe."</label></div>
			</div>
			<div class='row'>
				<div class='colonne3' align='right'>".$msg['caddie_contient_dont_inconnus']."</div>
				<div class='colonne3' align='center'><label class='etiquette' id='nb_item_blob'>".$this->nb_item_blob."</label></div>
				<div class='colonne_suite' align='center'><label id='nb_item_blob_pointe'>".$this->nb_item_blob_pointe."</label></div>
			</div>
			<div class='row'></div>
		</div>";
	}
