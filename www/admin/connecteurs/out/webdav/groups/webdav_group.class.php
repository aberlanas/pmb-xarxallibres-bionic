...
// +-------------------------------------------------+
// $Id: webdav_group.class.php,v 1.1 2016-02-05 10:06:14 vtouchard Exp $
		...
		foreach($this->config['tree'] as $pos => $elem){
			$result.="
						<tr id='tree_elem_tr".$pos."'>
							<td recept='yes' recepttype='tree_elem' highlight='tree_elem_show_recept' downlight='tree_elem_hide_recept' id='tree_elem_td".$pos."' draggable='yes' callback_after='move_tree_elem' dragtype='tree_elem' dragicon='$base_path/images/icone_drag_notice.png' dragtext='".htmlentities($group_collection[$elem], ENT_QUOTES, $charset)."'>
								<input type='hidden' name='tree[]' value='".$elem."' />
										<img src='".$base_path."/images/sort.png' style='width:12px; vertical-align:middle'/>".htmlentities($this->collections[$elem], ENT_QUOTES, $charset)."</td>
		...
		return "
						...
						function load_tree_elem(elem){
							...
							if(selected_option.value){
								...
								td.innerHTML = '<input type=\"hidden\" name=\"tree[]\" value=\"'+selected_option.value+'\" /> <img src=\"$base_path/images/sort.png\" style=\"width:12px; vertical-align:middle\"/>'+selected_option.innerHTML;
								tr.appendChild(td);
								var td = document.createElement('td');
