...
// +-------------------------------------------------+
// $Id: reindex_concept.inc.php,v 1.4.2.4 2017-12-14 16:45:23 vtouchard Exp $
...
if ($thesaurus_concepts_autopostage) {
	...
	$skos_controller = new onto_skos_controler($onto_index->handler, $params);
}
...
if($onto_index->handler->data_num_rows()) {
	...
	foreach($results as $row){	
		...
		if ($thesaurus_concepts_autopostage && !empty($skos_controller)) {
			$skos_controller->save_paths($row->item);
		}
	}
	$next = $start + $lot;
