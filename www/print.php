...
// +--------------------------------------------------------------------------+
// $Id: print.php,v 1.50.2.1 2017-10-12 12:25:24 jpermanne Exp $
...
if (($action_print=="")&&($_SESSION["PRINT"])) {
	...
	switch($environement['output']) {
		...
		case 'email':
			...
			if ($res_envoi) {
				print $pheader."<br /><br /><center><h3>".sprintf($msg["print_emailsucceed"],implode(', ',$mail_addresses))."</h3><br /><a href=\"\" onClick=\"self.close(); return false;\">".$msg["print_emailclose"]."</a></center></body></html>" ;
			} else {
				print $pheader."<br /><br /><center><h3>".sprintf($msg["print_emailfailed"],implode(', ',$mail_addresses))."</h3><br /><a href=\"\" onClick=\"self.close(); return false;\">".$msg["print_emailclose"]."</a></center></body></html>" ;
			}
			break;	
		case 'printer':
