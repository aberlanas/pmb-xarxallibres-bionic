...
// +-------------------------------------------------+
// $Id: pmbesClean.class.php,v 1.22.2.5 2018-02-09 09:45:40 dgoron Exp $
...
class pmbesClean extends external_services_api_class {
	...
	public function cleanConcept() {
		...
		if (SESSrights & ADMINISTRATION_AUTH) {
			...
			$onto_index = onto_index::get_instance();
			...
			if ($thesaurus_concepts_autopostage) {
				...
				$skos_controler = new onto_skos_controler($onto_index->handler, $params);
			}
			...
			if($onto_index->handler->data_num_rows()) {
				...
				foreach($results as $row){
					...
					//réindexation des entités liées avec les concepts autopostés.
					if ($thesaurus_concepts_autopostage && !empty($skos_controler)) {
						$skos_controler->save_paths($row->item);
						//index_concept::update_linked_elements(onto_common_uri::get_id($row->item));
					}
				}
			}
