...
// +-------------------------------------------------+
// $Id: ajax_selector.php,v 1.66.2.4 2018-02-02 09:54:40 vtouchard Exp $
	...
	case 'categories':
		...
		$requete="SELECT noeuds.id_noeud AS categ_id, noeuds.num_renvoi_voir as categ_see, noeuds.num_thesaurus, noeuds.not_use_in_indexation";
			if($thes_unique && (($lang==$thes->langue_defaut) || (in_array($lang, thesaurus::getTranslationsList())===false))){
				...
				$requete.=" FROM thesaurus JOIN noeuds ON thesaurus.id_thesaurus = noeuds.num_thesaurus JOIN categories as catdef on noeuds.id_noeud = catdef.num_noeud AND catdef.langue=thesaurus.langue_defaut JOIN categories as catlg on catdef.num_noeud=catlg.num_noeud and catlg.langue = '".$lang."' ".$equation_filters['join'];
		...
		if (!$aq->error) {
			...
			if($thes_unique && (($lang==$thes->langue_defaut) || (in_array($lang, thesaurus::getTranslationsList())===false))){
				...
				$requete1.=" FROM thesaurus JOIN noeuds ON thesaurus.id_thesaurus = noeuds.num_thesaurus JOIN categories as catdef on noeuds.id_noeud = catdef.num_noeud AND catdef.langue=thesaurus.langue_defaut JOIN categories as catlg on catdef.num_noeud=catlg.num_noeud and catlg.langue = '".$lang."' ".$equation_filters['join'];
		...
		if (isset($handleAs) && $handleAs == "json") {
			...
			if (pmb_mysql_num_rows($resultat) > 0) {
				...
				while($r=@pmb_mysql_fetch_array($resultat)) {
					$json_results[] = array("id" => $r[0], "datas" => $r[0], "value" => $r[1]);
				}
			}
		} else {
			$resultat=pmb_mysql_query($requete);		
	...
	case 'ARRAY':
		if (isset($handleAs) && ($handleAs == "json")) {
			...
			foreach ($array_selector as $index => $value) {
				...
				$json_results[] = array("id" => $categ_value, "datas" => $categ_value, "value" => $categ_ind);
			}
