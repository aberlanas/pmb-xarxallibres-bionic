...
// +-------------------------------------------------+
// $Id: indexint.inc.php,v 1.23.4.1 2018-01-18 08:30:14 wlair Exp $
...
if(!$opac_allow_affiliate_search || ($opac_allow_affiliate_search && $tab == "catalog")){
	print pmb_bidi("<h3 class='searchResult-search'><span class='searchResult-equation'><b>$count</b> $msg[indexint_found] <b>'".htmlentities(stripslashes($user_query),ENT_QUOTES,$charset)."'");
	...
	while($mesCategories = pmb_mysql_fetch_object($found)) {
		...
		print pmb_bidi("<a href=./index.php?lvl=indexint_see&id=".$categ_lien."&from=search><img src='".get_url_icon('folder.gif')."' border='0'/> ".$mesCategories->indexint_name." ".$mesCategories->indexint_comment."</a>");
		print "</li>";
		}
	print "</ul>";
	print "
