...
// +----------------------------------------------------------------------------------------+
// $Id: category.inc.php,v 1.42.2.1 2018-01-18 08:30:14 wlair Exp $
...
if(!$opac_allow_affiliate_search || ($opac_allow_affiliate_search && $tab == "catalog")){
	...
	print "
	<h3 class='searchResult-search'><span class='searchResult-equation'><b>$count</b> $msg[categs_found] <b>'".htmlentities(stripslashes($user_query),ENT_QUOTES,$charset)."'";
	...
	while($mesCategories_trouvees = pmb_mysql_fetch_object($found)) {
		...
		if ($mesCategories->num_parent) {
			...
			if ($bar[3]) print pmb_bidi("<a href=./index.php?lvl=categ_see&id=".$bar[3]['num_noeud']."><img src='".get_url_icon('folder.gif')."' border='0' align='middle'>...</a> > ");
			if ($bar[2]) print pmb_bidi("<a href=./index.php?lvl=categ_see&id=".$bar[2]['num_noeud']."><img src='".get_url_icon('folder.gif')."' border='0' align='middle'>".$bar[2]['libelle_categorie'].'</a> > ');
			if ($bar[1]) print pmb_bidi("<a href=./index.php?lvl=categ_see&id=".$bar[1]['num_noeud']."><img src='".get_url_icon('folder.gif')."' border='0' align='middle'>".$bar[1]['libelle_categorie'].'</a> > ');
		}
		...
		if(category::has_notices($mesCategories_trouvees->num_noeud))
			print " <img src='".get_url_icon('folder_search.gif')."' border=0 align='middle' />";
		else		
			print "<img src='".get_url_icon('folder.gif')."' border='0' align='middle'>";
