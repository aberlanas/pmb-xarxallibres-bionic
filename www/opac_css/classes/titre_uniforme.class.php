...
// +-------------------------------------------------+
// $Id: titre_uniforme.class.php,v 1.47 2017-07-19 12:06:19 dgoron Exp $
...
class titre_uniforme {
	...
	public function getData() {
		...
		if ($this->id) {
			...
			if (pmb_mysql_num_rows ( $result )) {
				...
				$this->p_perso = $authority->get_p_perso();
	...
	public function get_authors($id = 0) {
		...
		while ($resp_tu=pmb_mysql_fetch_object($res_sql)) {
			...
			/**
			 * Commenté en attente du passage des constantes gestion en opac dans vedette_link.class.php
			 */
			...
			$authority = new authority(0, $resp_tu->author_id, AUT_TABLE_AUTHORS);
			...
			$data = array( 
				...
				'objet' => $authority->get_object_instance()
			) ;
			$auteurs[] = $data;
...
// 		if(!file_exists($template_path)){
			$template_path =  $include_path.'/templates/authorities/common/isbd/titre_uniforme.html';
// 		}
		if(file_exists($include_path.'/templates/authorities/'.$opac_authorities_templates_folder."/isbd/titre_uniforme_subst.html")){
			$template_path =  $include_path.'/templates/authorities/'.$opac_authorities_templates_folder."/isbd/titre_uniforme_subst.html";
		}
		
		if($template_path){
			$h2o = new H2o($template_path);
			$authority = new authority(0, $this->id, AUT_TABLE_TITRES_UNIFORMES);
			$tu_isbd = $h2o->render(array('oeuvre' => $authority));
			return str_replace(array("\n", "\t", "\r"), '', strip_tags($tu_isbd));
		}
		return $this->tu_isbd;
	...
	public function format_link_datas($linked_oeuvre=array()) {
		$titre_uniforme = new titre_uniforme($linked_oeuvre['to_id']);
		return array_merge(
				array(
