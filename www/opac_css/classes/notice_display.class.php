...
// +-------------------------------------------------+
// $Id: notice_display.class.php,v 1.25.22.5 2018-01-25 15:45:18 dgoron Exp $
		...
		if (isset($mode_phototeque) && $mode_phototeque) {
			...
			if (pmb_mysql_num_rows($result)) {
				...
				if ($explnumobj->explnum_url) {
					$display .= "<center><img width='$opac_photo_mean_size_x' src=\"".$explnumobj->explnum_url."\"/></center><br />";
				} else{
					//répertoire d'upload ou stockage en base, le traitement reste identique...
					$display .= "<center><img src=\"vign_middle.php?explnum_id=".$explnumobj->explnum_id."\"/></center><br />";
				}
				if ($opac_photo_show_form) {
					$display .= "<center><a href='index.php?lvl=doc_command&id=".$this->id."&mode_phototeque=1'>".htmlentities($msg["command_phototeque_command_command"],ENT_QUOTES,$charset)."</a></center>";
				}
			}
			$hide_explnum=1;
		} else {
