...
// +-------------------------------------------------+
// $Id: author.class.php,v 1.42 2017-06-09 08:23:02 ngantier Exp $
	...
	public function getData() {
		...
		if ($this->id) {
			...
			if(pmb_mysql_num_rows($result)) {
				...
				$this->p_perso = $this->authority->get_p_perso();
				if($this->type==71 ) {
	...
	public function print_similar_name($nb_by_line=3) {
		...
		for($i=0;$i<$nb;$i++) {
			...
			if($data->nb_notice) {
				...
				$img_folder="<img src='".get_url_icon('folder_search.gif')."' border=0 align='absmiddle'>";	
			}else {
				$img_folder="<img src='".get_url_icon('folder.gif')."' border=0 align='absmiddle'>";
			}
