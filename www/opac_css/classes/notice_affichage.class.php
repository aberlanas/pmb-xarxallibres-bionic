...
// +-------------------------------------------------+
// $Id: notice_affichage.class.php,v 1.484.2.6 2018-01-26 15:37:55 dgoron Exp $
...
// 			}
	...
	public function get_display_collstates_bulletin_notice() {
		...
		if (pmb_mysql_num_rows($result)) {
			...
			if($collstate->nbr) {
				$display .= "<h3><span id='titre_exemplaires'>".$msg["perio_etat_coll"]."</span></h3>";
	...
	public function affichage_avis() {
		return $this->avis->get_display();
	} // fin affichage_avis


	public function avis_detail() {
		return $this->avis->get_display_detail();
	}
	...
	public function affichage_liste_lecture() {
		...
		return "<script type='text/javascript' src='./includes/javascript/liste_lecture.js'></script>
			...
			<a id='liste_lecture_display_tooltip_notice_".$this->notice_id."'><img src='".get_url_icon('liste_lecture.png')."' align='absmiddle' border='0' title=\"".$msg['notice_title_liste_lecture']."\" alt=\"".$msg['notice_title_liste_lecture']."\" /></a>
	...
	public function genere_ajax($aj_type_aff,$header_only_origine=0){
		...
		if($opac_notices_depliable == 2){
			...
			$template="$simili_search_script_all
				<div id=\"el!!id!!Parent\" class=\"notice-parent\">
				$case_a_cocher
		    	<img class='".$this->get_img_plus_css_class()."' src=\"./getgif.php?nomgif=plus\" name=\"imEx\" id=\"el!!id!!Img\" title=\"".$msg["expandable_notice"]."\" border=\"0\" param='".rawurlencode($this->notice_affichage_cmd)."' onClick=\"expandBase_ajax('el!!id!!', true,this.getAttribute('param')); $script_simili_search return false;\" hspace=\"3\"/>";
			if (!$this->no_header) {
				$template.=$this->get_icon_html($this->notice->niveau_biblio, $this->notice->typdoc);
			}
	    	$template.="
	...
	protected function get_resource_link_notice_header() {
		...
		$resource_link = "
			...
			<a href=\"".$this->notice->lien."\" target=\"_blank\" type='external_url_notice'>
				<img src=\"".get_url_icon("globe.gif", 1)."\" border=\"0\" align=\"middle\" hspace=\"3\" alt=\"".$info_bulle."\" title=\"".$info_bulle."\" />
			</a></span>";
	...
	public function genere_notice_childs() {
		...
		//Notices liées
		if(!$this->notice_childs && !$this->to_print && ($this->notice_relations->get_nb_childs() || $this->notice_relations->get_nb_pairs())) {
			$this->notice_childs = '';
// 			$affichage = '';
// 			if(!$this->seule) {
...
// 			}
// 			$affichage.="<br />";
			$display_links = $this->notice_relations->get_display_links('pairs', $this);
			$notice_pairs = '';
			foreach ($display_links as $relation_type=>$child_notices) {
			...
			$display_links = $this->notice_relations->get_display_links('childs', $this);
			$notice_childs = '';
			foreach ($display_links as $relation_type=>$child_notices) {
	...
	public function get_pdf_css_style() {
		$css_style = "
			<style type='text/css'>
				td.bg-grey {
					width:156;
