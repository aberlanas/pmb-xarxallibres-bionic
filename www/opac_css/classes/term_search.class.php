...
// +-------------------------------------------------+
// $Id: term_search.class.php,v 1.32 2017-07-12 15:15:00 tsamson Exp $
//
...
class term_search {
    ...
    public function page_navigator() {
    	...
    	if ($this->offset!=0) $navig.="<a href=\"$url_page&page=0"."&nbresultterme=".$this->n_total."\"><img src='".get_url_icon('first.png')."' border='0'/></a>"; else $navig.="<img src='".get_url_icon('first-grey.png')."' border='0'/>";
		if ($this->offset!=0) $navig.="<a href=\"$url_page&page=".($this->page-1)."&nbresultterme=".$this->n_total."&".$this->base_query."\"><img src='".get_url_icon('prev.png')."' border='0'/></a>"; else $navig.="<img src='".get_url_icon('prev-grey.png')."' border='0'/>";
		$navig.=" (".($this->offset+1)."-".min($this->offset+$this->n_per_page,$this->n_total).")/".$this->n_total." ";
		if (($this->offset+$this->n_per_page+1)<$this->n_total) $navig.="<a href=\"$url_page&page=".($this->page+1)."&nbresultterme=".$this->n_total."&".$this->base_query."\"><img src='".get_url_icon('next.png')."' border='0'/></a>"; else $navig.="<img src='".get_url_icon('next-grey.png')."' border=0/>";
		if (($this->offset+$this->n_per_page+1)<$this->n_total) $navig.="<a href=\"$url_page&page=$last_page"."&nbresultterme=".$this->n_total."\"><img src='".get_url_icon('last.png')."' border='0'/></a>"; else $navig.="<img src='".get_url_icon('last-grey.png')."' border='0'/>";
		return $navig;	
    }
    ...
    public function show_list_of_terms() {
		...
		while ($r=pmb_mysql_fetch_object($resultat)) {
			...
			if ($show) {
				...
				$res.= $nbre_termes."<a href=\"".$this->url_for_term_show.'?term='.rawurlencode($r->categ_libelle).'&id_thes='.$r->num_thesaurus.'&'.$this->base_query."\" target=\"term_show\" ".$result_com['java_com'].">";
