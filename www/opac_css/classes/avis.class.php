...
// +-------------------------------------------------+
// $Id: avis.class.php,v 1.22.2.1 2017-11-22 15:44:23 jpermanne Exp $
...
class avis {
	...
	protected function get_stars() {
		...
		if (!$this->public_average) {
			for ($i = 0; $i < 5; $i++) {
				$stars .= "<img class='img_star_avis' border=0 src='".get_url_icon('star_unlight.png')."' align='absmiddle' />";
			}
		} else {
			$cpt_star = 0;
			for ($i = 1; $i <= $this->public_average; $i++) {
				$stars.="<img class='img_star_avis' border=0 src='".get_url_icon('star.png')."' align='absmiddle' />";
				$cpt_star++;
			}
			if (substr($this->public_average,2,2) > 75) {
				...
				$stars .= "<img class='img_star_avis' border=0 src='".get_url_icon('star-semibright.png')."' align='absmiddle' />";
				$cpt_star++;
			}
	...
	protected function get_note_display_stars($note) {
		...
		$cpt_star = 4;
		for ($i = 1; $i <= $note; $i++) {
			$stars .= "<img border=0 src='".get_url_icon('star.png')."' align='absmiddle' alt=\"".$msg['avis_detail_note_'.$i]."\" />";
		}
		for ($i = round($note); $i <= $cpt_star; $i++) {
			$stars .= "<img border=0 src='".get_url_icon('star_unlight.png')."' align='absmiddle' alt=\"".$msg['avis_detail_note_'.$i]."\" />";
		}
		return $stars;
	}
