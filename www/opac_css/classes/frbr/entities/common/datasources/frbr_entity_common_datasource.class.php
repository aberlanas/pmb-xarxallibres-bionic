...
// +-------------------------------------------------+
// $Id: frbr_entity_common_datasource.class.php,v 1.6 2017-06-02 09:52:43 tsamson Exp $
	...
	public function get_form(){
				...
				$form .= "
					...
					<div class='colonne-suite'>
						<select name='datanode_sub_datasource_choice' data-pmb-evt='{\"class\":\"EntityForm\", \"type\":\"change\", \"method\":\"frbrEntityLoadElemForm\", \"parameters\":{\"id\":\"0\", \"domId\":\"sub_datasource_form\",\"filterRefresh\":\"1\",\"sortRefresh\":\"1\"}}'>
							<option value=''>".$this->format_text($this->msg['frbr_entity_common_datasource_sub_datasource_choice'])."</option>";
		...
		if ($this->limitable && isset($this->entity_type) && $this->entity_type && (count($sub_datasources)<=1)) {
			$form.= "
				<div class='row'>
					...
					<div class='colonne-suite'>
						<input type='text' name='datanode_datasource_nb_max_elements' value='".(isset($this->parameters->nb_max_elements) ? $this->parameters->nb_max_elements : '')."'/>
					</div>
				</div>";
		}
		if (isset($this->parameters->sub_datasource_choice) && $this->parameters->sub_datasource_choice) {
			$form.="<script type='text/javascript'>
						require(['dojo/topic'],					
						function(topic){
							topic.publish('ParametersFormsReady', 'frbrEntityLoadElemForm', {'elem' : '".$this->parameters->sub_datasource_choice."', 'id':'0', 'domId': 'sub_datasource_form', 'filterRefresh': '0', 'sortRefresh' :  '0'});	  
						});
					</script>";
		}
		$form.="</div>";
