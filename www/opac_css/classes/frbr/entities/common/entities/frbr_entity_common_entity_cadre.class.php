...
// +-------------------------------------------------+
// $Id: frbr_entity_common_entity_cadre.class.php,v 1.16.2.1 2017-09-20 10:01:20 vtouchard Exp $
...
// 					<input type='hidden' name='".$this->type."_num_datanode' id='".$this->type."_num_datanode' value='".$num_datanode."'/>
	...
	public function set_properties_from_form() {
		...
		$this->visible_in_graph = stripslashes($cadre_visible_in_graph);
		$this->display_empty_template = stripslashes($cadre_display_empty_template);
	...
	public function show_cadre($datanodes_data = array()) {
		if(isset($this->datanode) && is_object($this->datanode)) {
			if ($this->view['id'] != 0) {
				if (isset($datanodes_data[$this->datanode->get_id()]) && count($datanodes_data[$this->datanode->get_id()])) {
					...
					return "<div id='".$this->get_dom_id()."'".($this->css_class != '' ? " class='".$this->css_class."'" : "").">".$view->render($data)."</div>";
				} else {
					$view_data = "";
					if ($this->display_empty_template) {
						$view = new $this->view['name']($this->view['id']);
						$view_data = $view->render();
					}
					return "<div id='".$this->get_dom_id()."'".($this->css_class != '' ? " class='".$this->css_class."'" : "").">".$view_data."</div>";
				}
			}
