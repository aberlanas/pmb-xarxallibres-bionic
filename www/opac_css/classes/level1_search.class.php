...
// +-------------------------------------------------+
// $Id: level1_search.class.php,v 1.14.2.1 2018-01-26 15:37:55 dgoron Exp $
...
class level1_search {
    ...
    public function search_authors() {
		...
		$filter='';
		if ($typdoc || $add_notice) $filter = ' join responsability on responsability_author=author_id join notices on notice_id=responsability_notice ';
		if ($typdoc) $filter.=" and typdoc='".$typdoc."' ";		
		if ($add_notice) $filter.= ' and notice_id in ('.$add_notice.')';
    ...
    public function search_publishers() {
    ...
    public function search_titres_uniformes() {
    	global $opac_search_other_function,$typdoc,$charset,$dbh;
		...
		if ($opac_search_other_function) $add_notice=search_other_function_clause();		
		$filter='';
		if ($typdoc || $add_notice) $filter = ' join notices_titres_uniformes on ntu_num_tu = tu_id	join notices on ntu_num_notice = notice_id ';
		if ($typdoc) $filter.= "  and typdoc='".$typdoc."' ";
		if ($add_notice) $filter.= ' and notice_id in ('.$add_notice.')'; 
		$clause = $filter.$clause;
    ...
    public function search_subcollections() {
    	global $opac_search_other_function,$typdoc,$dbh,$charset;
    	// on regarde comment la saisie utilisateur se présente
		$clause = '';
		$add_notice = '';
    ...
    public function search_indexints() {
    	global $opac_search_other_function,$typdoc,$dbh,$charset;
    	global $opac_stemming_active;
    	// on regarde comment la saisie utilisateur se présente
		$clause = '';
    ...
    public function search_abstracts() {
    	global $typdoc,$dbh,$charset;
    ...
    public function search_concepts() {
    	global $opac_search_other_function,$typdoc,$charset,$dbh;
    	global $opac_stemming_active;
    	// on regarde comment la saisie utilisateur se présente
    	$clause = '';
