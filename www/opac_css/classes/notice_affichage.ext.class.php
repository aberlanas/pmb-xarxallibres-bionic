...
// +-------------------------------------------------+
// $Id: notice_affichage.ext.class.php,v 1.271.2.4 2018-01-26 15:37:55 dgoron Exp $
...
class notice_affichage_custom_bretagne extends notice_affichage {
	...
	public function do_public_line($label, $value, $css='') {
		...
		if($value) {
			...
			$this->notice_public .=
			"<tr class='tr_".$css."'>
					<td align='right' class='bg-grey'><span class='etiq_champ'>".$label."</span></td>
	...
	public function do_public($short=0,$ex=1) {
		...
		if($this->notice->subcoll_id) {
			...
			$collection = new collection($this->notice->coll_id);
			$this->collections[]=$collection;
			$this->do_public_line($msg['coll_start'], inslink($collection->isbd_entry,  str_replace("!!id!!", $this->notice->coll_id, $this->lien_rech_collection)).$affnocoll." ".$collection->collection_web_link, 'coll');
		}
...
class notice_affichage_mw extends notice_affichage {
	...
	public function do_public_line($label, $value, $css='') {
		if($value) {
			...
			$this->notice_public .=
			"<tr class='tr_".$css."'>
					<td align='right' class='bg-grey".$this->x."'><span class='etiq_champ'>".$label."</span></td>
...
class notice_affichage_mba extends notice_affichage {
	...
	public function get_aff_fields_perso() {
		...
		if($titre){
			$aff_fields_perso= "<tr><td align='right' class='bg-grey'><span class='etiq_champ'>".$msg['n_titre_de_forme']."</span></td><td>".$titre."</td></tr>" ;
			$lieu=explode("/",$lieu_ed);
			if(count($lieu)){
				for($l=0;$l<count($lieu);$l++){
					$aff_fields_perso.= "<tr><td align='right' class='bg-grey'><span class='etiq_champ'></span></td><td>".$lieu[$l]."</td></tr>" ;
				}
			}
			
			if($date){
				$aff_fields_perso.= "<tr><td align='right' class='bg-grey'><span class='etiq_champ'></span></td><td>".$date."</td></tr>" ;
			}
		}
...
//		}
	...
	public function get_aff_fields_perso() {
			...
			for ($i=0; $i<count($this->memo_perso_["FIELDS"]); $i++) {
				...
				if ($p['OPAC_SHOW'] && $p["AFF"]){
					if ($p["NAME"] != "cp_mc_eng" && $p["NAME"] != "cp_vol_per" && $p["NAME"] != "ESCR") {
						$aff_fields_perso .="<tr><td align='right' class='bg-grey'><span class='etiq_champ'>".strip_tags($p["TITRE"])."</span></td><td>".$p["AFF"]."</td></tr>";
		...
		if(trim($libKeywords)){
			$keywords .= "<tr><td align='right' class='bg-grey'><span class='etiq_champ'>".$libKeywords."</span></td><td>".$valKeywords."</td></tr>";
		}
	...
	public function aff_parents() {
		...
		$retour = '';
		$r_type=array();
		if(count($this->notice_relations->get_nb_parents())) {
			if ($opac_notice_affichage_class) $notice_affichage=$opac_notice_affichage_class; else $notice_affichage="notice_affichage";
			$parents = $this->notice_relations->get_parents();
			foreach ($parents as $relation_type=>$parents_relations) {
				foreach ($parents_relations as $parent) {
					...
					if (!$r_type[$relation_type]) {
						...
						$retour.="<tr><td align='right' class='bg-grey'><span class='etiq_champ'>".notice_relations::$liste_type_relation['up']->table[$relation_type]."</span></td>";
					}
					$retour .= "<td>";
	...
	public function genere_simple($depliable=1, $what='ISBD') {
		...
		if ($depliable == 1) { 
			$template="<br />".$simili_search_script_all."
				...
				<table><tr><td width='8%'>
				$case_a_cocher
	    		<img class='".$this->get_img_plus_css_class()."' src=\"./getgif.php?nomgif=plus\" name=\"imEx\" id=\"el!!id!!Img\" title=\"".$msg['expandable_notice']."\" border=\"0\" onClick=\"expandBase('el!!id!!', true); $script_simili_search return false;\" hspace=\"3\"/>";
			if (!$this->no_header) {
			...
			$template="<br />".$simili_search_script_all."
	    		...
	    		<img class='".$this->get_img_plus_css_class()."' src=\"./getgif.php?nomgif=plus&optionnel=1\" name=\"imEx\" id=\"el!!id!!Img\" title=\"".$msg["expandable_notice"]."\" border=\"0\" hspace=\"3\"/>";
			if (!$this->no_header) {
				$template.=$this->get_icon_html($this->notice->niveau_biblio, $this->notice->typdoc);
			}
    		$template.= "&nbsp;".$basket;
	...
	protected function get_resource_link_notice_header() {
		...
		$resource_link = "
			...
			<a href=\"".$this->notice->lien."\" target=\"_blank\" type='external_url_notice'>
				<img src=\"".get_url_icon("lien-web_x16.png", 1)."\" border=\"0\" align=\"middle\" hspace=\"3\" alt=\"".$info_bulle."\" title=\"".$info_bulle."\" />
			</a></span>";
		return $resource_link;
	}
	...
	protected function genere_childs_relation($relation_type, $child_notices) {
		...
		foreach ($child_notices as $i=>$child_data) {
			if(($i<20) || $this->seule) {
				...
				$notice_childs .= "
					<br /><table><tr>
						<td width='3%'><li style='list-style-type: none;'>".$html_icon."</td>
						<td width='87%'>
							<a href='".str_replace("!!id!!",$child_data['linked_notice'],$this->lien_rech_notice)."'>".$memo_notice[$child_data['linked_notice']]["header_without_doclink"]."</a>
						</td>
						<td>".$child_data['display']."</td>
						</tr></table>";
			} else {
				break;
			}
