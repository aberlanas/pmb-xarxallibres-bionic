...
// +-------------------------------------------------+
// $Id: liste_lecture.class.php,v 1.48.2.1 2018-01-25 14:32:40 dgoron Exp $
...
class liste_lecture {
	...
	public function aff_navigation_notices($notices=array(),$id_liste, $sub){
		...
		if($precedente > 1) {
			$affichage .= "<td width=\"14\" align=\"center\"><a href=\"index.php?lvl=show_list&sub=$sub&id_liste=$id_liste&page=1\"><img src=\"".get_url_icon('first.png')."\"";
			$affichage .= " border=\"0\" alt=\"$msg[start]\"";
			$affichage .= " title=\"$msg[first_page]\"></a></td>";
		} else {
			$affichage .= "<td width=\"14\" align=\"center\"><img src=\"".get_url_icon('first-grey.png')."\">";
		}
	
		if($precedente > 0) {
			$affichage .= "<td width=\"14\" align=\"center\"><a href=\"index.php?lvl=show_list&sub=$sub&id_liste=$id_liste&page=$precedente\"><img src=\"".get_url_icon('prev.png')."\"";
			$affichage .= " border=\"0\" alt=\"$msg[prec]\"";
			$affichage .= " title=\"$msg[prec]\"></a></td>";
		} else {
			$affichage .= "<td width=\"14\" align=\"center\"><img src=\"".get_url_icon('prev-grey.png')."\">";
		}
		...
		if($suivante<=$nbepages) {
			$affichage .= "<td width=\"14\" align=\"center\"><a href=\"index.php?lvl=show_list&sub=$sub&id_liste=$id_liste&page=$suivante\"><img src=\"".get_url_icon('next.png')."\"";
			$affichage .= " border=\"0\" alt=\"$msg[next]\"";
			$affichage .= " title=\"$msg[next]\"></a></td>";
		} else {
			$affichage .= "<td width=\"14\" align=\"center\"><img src=\"".get_url_icon('next-grey.png')."\">";
		}
	
		// affichage du lien vers la fin
		if($suivante < $nbepages) {
			$affichage .= "<td width=\"14\" align=\"center\"><a href=\"index.php?lvl=show_list&sub=$sub&id_liste=$id_liste&page=$nbepages\"><img src=\"".get_url_icon('last.png')."\"";
			$affichage .= " border=\"0\" alt=\"$msg[end]\"";
			$affichage .= " title=\"$msg[end]\"></a></td>";
		} else {
			$affichage .= "<td width=\"14\" align=\"center\"><img src=\"".get_url_icon('last-grey.png')."\">";
		}
	...
	public function get_display_empr() {
		...
		if(count($this->empr)) {
			foreach ($this->empr as $empr) {
				$display .= "<img border=0 align='top' src='".get_url_icon('cross.png', 1)."'  onclick=\"delete_from_liste('".$this->id_liste."','".$empr->id_empr."');\">";
				$display .= $empr->nom."<br />";
			}
		} else {
...
// 		$display = gen_liste($query,'id_liste','nom_liste', 'listes_lecture_notice_'.$notice_id, 'liste_lecture_add_notice(this.value, '.$notice_id.'); return false;', '', 0, $msg['avis_liste_lecture_empty'], 0, $msg['notice_title_liste_lecture_default_value']);
	...
	protected function gen_selector_tags() {
		...
		$display = "<option value='' ".(!$this->tag ? "selected='selected'" : "")."></option>";
		$tags = $this->get_tags();
		if(count($tags)){
