...
// +-------------------------------------------------+
// $Id: facettes_root.class.php,v 1.22.2.4 2018-01-26 15:39:32 jpermanne Exp $
...
abstract class facettes_root {
	...
	protected function facette_existing(){
		...
		$this->facettes = array();
		$query = "SELECT * FROM ".static::$table_name." WHERE facette_visible=1 ORDER BY facette_order, facette_name";
		...
		if (count($expls_location['ids']) > 1) {
			...
			if (pmb_mysql_num_rows($result)) {
				...
				while ($row = pmb_mysql_fetch_object($result)) {
					...
					$tab_locations[$i]['url'] = $opac_url_base.'index.php?lvl=more_results&mode=extended&facette_test=1';
					...
					if($row->surloc_num && !array_key_exists($row->surloc_num,$tab_surlocations) && $opac_sur_location_activate) {
						...
						$tab_surlocations[$row->surloc_num]['url'] = $opac_url_base.'index.php?lvl=more_results&mode=extended&facette_test=1';
	...
	protected function get_display_not_clicked() {
		...
		if(is_array($this->not_clicked) && count($this->not_clicked)) {
			...
			$display_not_clicked .= "<input class='bouton bouton_filtrer_facette_haut' type='button' value='".htmlentities($msg["facette_filtre"],ENT_QUOTES,$charset)."' name='filtre2' onClick='valid_facettes_multi()'>";
	...
	public function create_ajax_table_facettes(){
		...
		$table = "<form name='facettes_multi' class='facettes_multis' method='POST' action='".$base_path."/index.php?lvl=more_results&mode=".$this->mode."&facette_test=1'>";
