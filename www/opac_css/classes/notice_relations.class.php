...
// +-------------------------------------------------+
// $Id: notice_relations.class.php,v 1.10 2017-07-12 15:15:00 tsamson Exp $
...
class notice_relations {
	...
	protected static function parse() {
		if (!isset(static::$liste_type_relation)) {
			...
			static::$liste_type_relation['both'] = new marc_list("relationtypedown");
			...
			foreach (static::$liste_type_relation['both']->attributes as $key=>$reverse) {
				if(!isset(static::$corresp_relation_up_down[$reverse['REVERSE_CODE']])) {
	...
	protected function fetch_data() {
		...
		$query = "select id_notices_relations, num_reverse_link, n2.niveau_biblio 
	...
	public function get_display_links($type_links, $notice_affichage) {
		...
		if($type_links == 'childs' && $notice_affichage->notice->niveau_biblio == 'b') {
			$analysis = $this->get_analysis();
			$relations_links = array();
			foreach ($analysis as $link) {
				if(!$notice_affichage->seule) {
					$relations_links[] = $link->get_display_link($notice_affichage, 1);
				} else if (!isset($notice_affichage->antiloop[$link->get_linked_notice()])) {
					$relations_links[] = $link->get_display_link($notice_affichage, $header_only);
				}
			}
			$display_links[static::$liste_type_relation['down']->table['d']] = $relations_links;
		}
	...
	public static function get_access_rights() {
		...
		if(!isset(static::$access_rights)) {
			...
			if (is_null($dom_2)) {
				...
				static::$access_rights["statut_r"] = "and notices.statut=id_notice_statut and ((notice_visible_opac=1 and notice_visible_opac_abon=0)".($_SESSION["user_code"]?" or (notice_visible_opac_abon=1 and notice_visible_opac=1)":"").")";
			} else {
