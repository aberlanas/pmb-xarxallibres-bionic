...
// +-------------------------------------------------+
// $Id: search.class.php,v 1.230.2.17 2018-02-15 16:08:08 apetithomme Exp $
...
class search {
	...
	protected function parse_search_file() {
		...
		if(!$dejaParse){
			...
			if($this->fichier_xml == 'search_fields_authorities_gestion') {
				if(!$this->keyName) {
	...
	protected function get_completion_selection_field($i,$n,$search, $v, $params=array()) {
		...
		$r.= "<input class='bouton' value='...' title='".htmlentities($msg['title_select_from_list'],ENT_QUOTES,$charset)."' id='$fnamesans"."_selection_selector' onclick=\"openPopUp('./select.php?what=$selector&caller=search_form&$p1=".$fname_id."_0&$p2=".$fnamesanslib."_0&search_xml_file=".$this->fichier_xml."&search_field_id=".$search."&deb_rech=&callback=selectionSelected&infield=".$fnamesans."_0', 'select_selection', 400, 400, -2, -2, 'scrollbars=yes, toolbar=no, dependent=yes, resizable=yes')\" type=\"button\">";
		...
		for($inc=0;$inc<$nb_values;$inc++){
			...
			$r.="<span class='search_dico'><img src='".get_url_icon("dictionnaire.png")."' align='middle' onClick=\"document.getElementById('".$fnamesanslib."_".$inc."').focus();simulate_event('".$fnamesanslib."_".$inc."');\"></span>";
...
// 			$fnamevar_id_js = "fieldvar_".$n."_".$search."[id_scheme][]";
		...
		$r= "<input type='hidden' id='$fnamesans"."_max_aut' value='".$nb_max_aut."'>";
		$r.= "<input class='bouton' value='...' id='$fnamesans"."_authority_selector' title='".htmlentities($msg['title_select_from_list'],ENT_QUOTES,$charset)."' onclick=\"openPopUp('./select.php?what=$selector&caller=search_form&mode=un&$p1=".$fname_id."_0&$p2=".$fnamesanslib."_0&deb_rech=&callback=authoritySelected&infield=".$fnamesans."_0".($params['selector'] == "ontology" ? '&dyn=4' : '')."', 'select_author0', 400, 400, -2, -2, 'scrollbars=yes, toolbar=no, dependent=yes, resizable=yes')\" type=\"button\">";
		...
		if(!$this->groups_used){
			...
			foreach($this->groups as $group_id => $group){
				if(isset($fields_array[$group_id])){ //On a des champs définis pour le groupe courant
					$r.="<optgroup label='".htmlentities($this->groups[$group_id]['label'],ENT_QUOTES,$charset)."' class='erreur'>\n";
			...
			if(count($lonely_fields)){
				$r.= "<optgroup label='".htmlentities($msg["search_extended_lonely_fields"], ENT_QUOTES, $charset)."' class='erreur'>\n";
	...
	public function make_htmlentities_test_array($value){
		global $charset;
	
		//htmlentities récursif car les facette sont des tableaux de tableaux
		if (is_array($value)) {
			foreach ($value as $k=>$v) {
				$value[$k] = $this->make_htmlentities_test_array($value[$k]);
			}
			return $value;
		} else {
	...
	public function serialize_search($recurse_history=false, $need_htmlentities=false) {
		...
		if($need_htmlentities){
			$to_serialize = $this->make_htmlentities_test_array($to_serialize);
		}
	...
	protected function gen_temporary_table($table_name, $main='', $with_pert=false) {
		...
		@pmb_mysql_query($requete);
		if($with_pert && !(pmb_mysql_num_rows(pmb_mysql_query('show columns from '.$table_name.' like "pert"')))) {
