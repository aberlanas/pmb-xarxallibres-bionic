...
// +-------------------------------------------------+
// $Id: authority_page.class.php,v 1.13.2.6 2018-02-09 11:05:37 dgoron Exp $
...
class authority_page {	
	...
	public function get_recordslist($template_directory = "") {
		...
		$recordslist = "<h3><span class=\"aut_details_liste_titre\">".$this->get_title_recordslist()."</span></h3>\n";
		...
		if($nbr_lignes) {
			...
			while(($obj=pmb_mysql_fetch_object($res))) {
				...
				$recordslist.= pmb_bidi(aff_notice($obj->notice_id, 0, 1, 0, "", "", 0, 0, $recherche_ajax_mode, 1, $template_directory));
...
// 			$recordslist.= "</div><!-- fermeture #aut_details_liste -->\n";
			$recordslist.= "<div id='navbar'><hr /><center>".printnavbar($page, $nbr_lignes, $opac_nb_aut_rec_per_page, "./index.php?lvl=".$this->get_mode_recordslist()."&id=".$this->id."&page=!!page!!&nbr_lignes=$nbr_lignes&l_typdoc=".rawurlencode($l_typdoc).($nb_per_page_custom ? "&nb_per_page_custom=".$nb_per_page_custom : ''))."</center></div>\n";
		} else {
