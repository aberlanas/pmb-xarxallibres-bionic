...
// +-------------------------------------------------+
// $Id: facette_search.class.php,v 1.88.2.1 2017-10-25 15:15:30 dgoron Exp $
...
class facettes extends facettes_root {
	...
	protected function get_query_by_facette($id_critere, $id_ss_critere) {
		...
		$query = "select value ,count(distinct id_notice) as nb_result from (SELECT value,id_notice FROM notices_fields_global_index
					".gen_where_in('notices_fields_global_index.id_notice', $this->objects_ids)."
	...
	protected static function get_link_delete_clicked($id, $facettes_nb_applied) {
		if ($facettes_nb_applied==1) {
			$link = "document.location=\"index.php?lvl=more_results&get_last_query=1&reinit_facette=1\";";
		} else {
			$link = "document.location=\"index.php?lvl=more_results&mode=extended&facette_test=1&param_delete_facette=".$id."\";";
		}
		return $link;
	}
	...
	protected static function get_link_not_clicked($name, $label, $code_champ, $code_ss_champ, $id, $nb_result) {
		global $base_path;
		$link =  "document.location=\"".$base_path."/index.php?lvl=more_results&mode=extended&facette_test=1";
	...
	protected static function get_link_back($reinit_compare=false) {
		global $base_path;
		$link =  "document.location.href=\"".$base_path."/index.php?lvl=more_results&get_last_query=1".($reinit_compare ? "&reinit_compare=1" : "")."\"";
		return $link;
	}
