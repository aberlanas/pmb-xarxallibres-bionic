...
// +-------------------------------------------------+
// $Id: scan_request.class.php,v 1.11 2017-03-23 14:13:36 jpermanne Exp $
...
class scan_request {
	...
	public function send_mail(){
		global $charset, $msg;
		global $pmb_scan_request_location_activate, $empr_location;
		...
		if ($pmb_scan_request_location_activate) {
			$location = new docs_location($empr_location);
			if ($location->email) {
	...
	public function save() {
		...
		if($this->id) {
			...
			$query .= 'scan_request_create_date=now(),
					...
					scan_request_num_location='.$empr_location.',
	...
	public function get_form_in_record($record_id, $record_type = 'notices') {
		...
		if ($_SESSION['id_empr_session']) {
			...
			$display = str_replace("<!--bouton close-->","<a href='#' onClick='parent.kill_scan_request_frame();return false;'><img src='".get_url_icon('close.gif')."' border='0' align='right'></a></div>", $display);
			if($this->id){
				$display = str_replace('!!form_title!!', htmlentities($msg['scan_request_edit_form'], ENT_QUOTES, $charset), $display);
			} else {
				$display = str_replace('!!form_title!!', htmlentities($msg['scan_request_create_form'], ENT_QUOTES, $charset), $display);
		...
		if(count($this->linked_records)) {
			foreach ($this->linked_records as $linked_record) {
				...
				if(!$linked_record['scannable']) {
					...
					$label = '<strike title="'.$msg['scan_request_linked_record_no_scannable'].'">'.strip_tags($linked_record['display']).'</strike>';
				} else {
					$expand_invisible = '';
