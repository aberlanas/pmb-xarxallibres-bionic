...
// +-------------------------------------------------+
// $Id: notice_relation.class.php,v 1.5.2.2 2018-02-09 16:01:40 dgoron Exp $
...
class notice_relation {
	...
	protected function get_drag_template($id_elt, $tit1='') {
		...
		$drag_link = "<div id=\"drag_".$id_elt."\" handler=\"handle_".$id_elt."\" dragtype='".$dragtype."' draggable='yes' recepttype='".$recepttype."' recept='yes'
									dragicon=\"".$base_path."/images/icone_drag_notice.png\" dragtext=\"".htmlentities($tit1,ENT_QUOTES,$charset)."\" callback_before=\"is_expandable\"
		...
		$drag_link .= "<span id=\"handle_".$id_elt."\" style=\"float:left; padding-right : 7px\"><img src=\"".$base_path."/images/sort.png\" style='width:12px; vertical-align:middle' /></span>";
		return $drag_link;
	}
	...
	public function get_display_link($notice, $header_only=1) {
		...
		if($header_only) {
			...
			if ($notice->get_parents_header_without_html()) {
				...
				$display_link .= $memo_notice[$this->linked_notice]["header_without_doclink"];
			}
