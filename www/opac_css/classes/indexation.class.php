...
// +-------------------------------------------------+
// $Id: indexation.class.php,v 1.16.2.4 2018-01-24 10:46:39 dgoron Exp $
...
class indexation {
	...
	public function __construct($xml_filepath, $table_prefix, $type = 0){
		...
		if(!isset(static::$xml_indexation[$this->type]) || !count(static::$xml_indexation[$this->type])) {
			...
			fclose($fp);
			static::$xml_indexation[$this->type]=_parser_text_no_function_($xml,"INDEXATION");
		}
	}
						...
						case "nn" :
							...
							if (isset($link["TABLEKEY"][0]['value'])) {
								...
								if(isset($link['LINK'][0])) {
									...
									$jointure .= " JOIN " . $table['NAME'] . (isset($table['ALIAS'])? " as ".$table['ALIAS'] :"");
									$jointure .= " ON (" . $link['LINK'][0]['TABLE'][0]['value'] . "." . $link['LINK'][0]['EXTERNALFIELD'][0]['value'];
	...
	public function maj($object_id,$datatype='all'){	
		...
		if ($this->champ_trouve) {
								...
								if($liste_mots!='') {
									...
									if(!isset($this->tab_code_champ[$k][$nom_champ]['use_global_separator']) || !($this->tab_code_champ[$k][$nom_champ]['use_global_separator'])){
										$tab_fields[$nom_champ][] = array(
												'value' =>trim($liste_mots),
												'lang' => $langage,
												'autorite' => (isset($this->tab_code_champ[$k][$nom_champ]['autorite']) && isset($tab_row["subst_for_autorite_".$this->tab_code_champ[$k][$nom_champ]['autorite']]) ? $tab_row["subst_for_autorite_".$this->tab_code_champ[$k][$nom_champ]['autorite']] : '')
										);
									} else {
			...
			// Les champs perso
			if(count($this->tab_pp) && ($datatype == 'all' || in_array('custom_field',$this->datatypes[$datatype]))){
			...
			//Les autorités perso
			if(count($this->tab_authperso) && ($datatype == 'all' || in_array('authperso',$this->datatypes[$datatype]))){
				$order_fields=1;
	...
	public static function get_indexation_sub_join($link) {
		...
		switch ($link["TYPE"]) {
			...
			case "nn" :
				...
				$jointure .= " ON (" . $link["TABLE"][0]['value'] . "." .  $link['EXTERNALFIELD'][0]['value'];
