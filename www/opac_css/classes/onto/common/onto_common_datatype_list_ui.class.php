...
// +-------------------------------------------------+
// $Id: onto_common_datatype_list_ui.class.php,v 1.7.2.1 2017-09-14 14:45:23 tsamson Exp $
	...
	public static function get_form($item_uri,$property, $restrictions,$datas, $instance_name,$flag) {
		...
		if (isset($property->pmb_list_query)) {
			...
			while ($row = pmb_mysql_fetch_object($result)) {
				$options_values[$row->id] = $row->value;
			}
		}
		...
		if(sizeof($datas)){			
			...
			foreach($options_values as $id => $value){
				if ($restrictions->get_max() != 1) {
					...
					$options.= htmlentities('{value:"'.$id.'", label:"'.$value.'"'.(in_array($id, $values) ? ', selected : true' : '').'}', ENT_QUOTES, $charset);
				}
			}
			...
			foreach($options_values as $id => $value){
				if ($restrictions->get_max() != 1) {
					...
					$options.= htmlentities('{value:"'.$id.'", label:"'.$value.'"}', ENT_QUOTES, $charset);
				}
			}			
