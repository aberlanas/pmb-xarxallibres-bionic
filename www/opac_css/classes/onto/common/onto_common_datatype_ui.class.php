...
// +-------------------------------------------------+
// $Id: onto_common_datatype_ui.class.php,v 1.3.2.2 2017-12-27 09:47:01 apetithomme Exp $
...
public static function get_validation_js($item_uri,$property, $restrictions,$datas, $instance_name,$flag){
	...
	public static function get_form_with_special_properties($property,$datas, $instance_name, $form) {
		if (!$property->pmb_extended['readonly']) {
			$form = str_replace("!!onto_disabled!!","",$form);
			$form = str_replace("!!onto_readonly!!","",$form);
			return $form;
		}
		
		if (strpos($form,'!!onto_disabled!!')) {
			$form = str_replace("!!onto_disabled!!","disabled : true",$form);
			/**
		...
		$form = str_replace("!!onto_readonly!!","readOnly : true",$form);
		return $form;
	}
