...
// +-------------------------------------------------+
// $Id: notice_affichage_ctles.class.php,v 1.25.2.3 2017-09-13 09:29:34 dgoron Exp $
...
class notice_affichage_ctles extends notice_affichage {
				...
				foreach ($parents_relations as $parent) {
					...
					/*if (count($this->notice_relations->get_nb_parents())==1) {
						...
						if (!$r_type[$relation_type]) {
							...
							$this->parents.="<tr><td align='left' class='bg-grey'><span class='etiq_champ'>".notice_relations::$liste_type_relation['up']->table[$relation_type]."</span></td><td>\n";
	...
	protected function genere_childs_relation($relation_type, $child_notices) {
		...
		$notice_childs = "<tr><td align='left' class='bg-grey'><span class='etiq_champ'>".$relation_type." :</span></td>";
		if (!$this->seule) {
			$notice_childs .= "<td>";
		}
