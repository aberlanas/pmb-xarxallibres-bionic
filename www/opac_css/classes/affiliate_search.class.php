...
// +-------------------------------------------------+
// $Id: affiliate_search.class.php,v 1.19.2.1 2017-09-18 13:24:09 dgoron Exp $
...
class affiliate_search {
	...
	protected function getNavigationBar($nb_results=0) {
		...
		if(!$page) $page = 1;
		if($nb_results) {
			$nbr_lignes = ceil($nb_results/$opac_search_results_per_page);
		} else {
			$nbr_lignes = ceil($this->getTotalNbResults()/$opac_search_results_per_page);
		}
		...
		$navigationBar .= printnavbar($page, $nbr_lignes, $opac_search_results_per_page, $url_page, $nb_per_page_custom_url, $action).
		$navigationBar .= "</center></div>";
   ...
   public function getAuthoritiesResults(){
    	...
    	if($this->table_tempo){
 			if(!$this->authoritiesResult){
	    		...
	    		for($i = $start_page ; $i<$last_item ; $i++){
	    			$this->authoritiesResult.= "
					<li><a href='#' onclick='document.form_values.action=\"./index.php?lvl=external_authorities&type=".$this->type."&ext_value=".urlencode($indexint[$i])."\";document.form_values.submit();return false;'><img border='0' src='".get_url_icon('folder.gif')."'> ".$indexint[$i]."</a></li>";
	    		}												
    ...
    public function fetch_indexint(){
		global $dbh ;
		global $opac_url_base ;
...
class affiliate_search_publisher extends affiliate_search {
   ...
   public function getAuthoritiesResults(){
	...
	public function fetch_publishers() {
		global $dbh ;
		global $opac_url_base ;
...
class affiliate_search_subcollection extends affiliate_search {
   ...
   public function getAuthoritiesResults(){
	...
	public function fetch_subcollections() {
		global $dbh ;
...
class affiliate_search_concept extends affiliate_search {
   ...
   public function getAuthoritiesResults(){
	...
	public function fetch_concepts() {
		global $dbh ;
		global $opac_url_base ;
