...
// +-------------------------------------------------+
// $Id: demandes.class.php,v 1.44.2.2 2017-10-12 12:58:42 jpermanne Exp $
...
// 				}
				...
				$liste .= "<td class='empr_demandes_col1'><img hspace=\"3\" border=\"0\" onclick=\"expand_action('action".$dmde->id_demande."','$dmde->id_demande', true); return false;\" title=\"\" id=\"action".$dmde->id_demande."Img\" name=\"imEx\" class=\"img_plus\" src=\"".get_url_icon("plus.gif")."\"></td>";
				...
				if(!$opac_demandes_affichage_simplifie) {
					...
					<td class='empr_demandes_col_progression'>
						<img src=\"$base_path/images/jauge.png\" height='15px' width=\"".$dmde->progression."%\" alt='".$dmde->progression."%' />
					</td>";
					$perso_=$p_perso->show_fields($dmde->id_demande);
					if(isset($perso_["FIELDS"])) {
						for ($i=0; $i<count($perso_["FIELDS"]); $i++) {
							$p=$perso_["FIELDS"][$i];
							if($p["OPAC_SHOW"]) $liste .= "<td class='empr_demandes_col_".$p["NAME"]."'>".($p["TYPE"]=='html'?$p["AFF"]:nl2br($p["AFF"]))."</td>";
						}
					}
					$liste .="<td>";
					if ($dmde->get_num_linked_notice()) {
