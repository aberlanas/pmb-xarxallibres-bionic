...
// +-------------------------------------------------+
// $Id: more_results.inc.php,v 1.96.2.1 2017-09-18 14:33:48 dgoron Exp $
...
if (stristr($_SERVER['REQUEST_URI'], ".inc.php")) die("no access");
	...
	// affichage recherche
	$clause = (isset($clause) ? stripslashes($clause) : '');
	$tri = (isset($tri) ? stripslashes($tri) : '');
	$pert = (isset($pert) ? stripslashes($pert) : '');
	$clause_bull = (isset($clause_bull) ? stripslashes($clause_bull) : '');
	$clause_bull_num_notice = (isset($clause_bull_num_notice) ? stripslashes($clause_bull_num_notice) : '');
	$join = (isset($join) ? stripslashes($join) : '');
...
*/
	...
	$catal_navbar .= "<div id='navbar'><hr />\n<center>".printnavbar($page, $count, $opac_search_results_per_page, $url_page, $nb_per_page_custom_url, $action)."</center></div>";

	$active_facette = 0;
	$active_facettes_external = 0;
	$nav_displayed = (isset($recordmodes) ? $recordmodes->is_nav_displayed($recordmodes->get_current_mode()) : true);
	...
	if(($active_facette)&& ($tab!="affiliate")){
		...
		if(isset($reinit_facette) && $reinit_facette) unset($_SESSION['facette']); 
...
switch ($search_type) {
	...
	case 'tags_search':
		...
		$form .= "<input type=\"hidden\" name=\"nbexplnum_to_photo\" value=\"".(isset($nbexplnum_to_photo) ? $nbexplnum_to_photo : '')."\">\n";
		$form .= "</form>";
