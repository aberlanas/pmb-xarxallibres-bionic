...
// +-------------------------------------------------+
// $Id: bulletin_display.inc.php,v 1.75.2.2 2018-02-01 10:07:50 jpermanne Exp $
	...
	if(pmb_mysql_num_rows($res_caroussel)){
		...
		for($i=0 ; $i<($opac_navigateur_bulletin_number-sizeof($previous)); $i++){
			if($i<($opac_navigateur_bulletin_number-sizeof($previous))-1)
				$carroussel .="<td style='width:".($taille/(($opac_navigateur_bulletin_number*2)+1))."%;'>&nbsp;</td>";
			else{
				if(!$link_perio=get_perio_link($bull['bulletin_notice'],'before'))$carroussel .="<td style='width:".($taille/(($opac_navigateur_bulletin_number*2)+1))."%;'>&nbsp;</td>";
				else $carroussel .="<td class='active' style='width:".($taille/(($opac_navigateur_bulletin_number*2)+1))."%;'>$link_perio</td>";	
			} 
		}
		
		if(sizeof($previous)>0){
			for($i=sizeof($previous)-1 ; $i>=0 ; $i--){
				$carroussel .="<td class='active' style='width:".($taille/(($opac_navigateur_bulletin_number*2)+1))."%;'><a href='index.php?lvl=bulletin_display&id=".$previous[$i]['bulletin_id']."'>".$previous[$i]['bulletin_numero'].($previous[$i]['bulletin_titre'] ? " - ".$previous[$i]['bulletin_titre'] : "")."<br />".($previous[$i]['mention_date'] ? $previous[$i]['mention_date'] :$previous[$i]['aff_date_date'] )."</a></td>";
			}
		}
		//le bull courant en évidence
		$carroussel .="<td class='current_bull_carroussel' style='width:".($taille/(($opac_navigateur_bulletin_number*2)+1))."%;'><a href='index.php?lvl=bulletin_display&id=".$current['bulletin_id']."'>".$current['bulletin_numero'].($current['bulletin_titre'] ? " - ".$current['bulletin_titre'] : "")."<br />".($current['mention_date'] ? $current['mention_date'] :$current['aff_date_date'] )."</a></td>";
		//la suite
		if(sizeof($next)>0){
			for($i=0 ; $i<sizeof($next) ; $i++){
				$carroussel .="<td class='active' style='width:".($taille/(($opac_navigateur_bulletin_number*2)+1))."%;'><a href='index.php?lvl=bulletin_display&id=".$next[$i]['bulletin_id']."'>".$next[$i]['bulletin_numero'].($next[$i]['bulletin_titre'] ? " - ".$next[$i]['bulletin_titre'] : "")."<br />".($next[$i]['mention_date'] ? $next[$i]['mention_date'] :$next[$i]['aff_date_date'] )."</a></td>";
			}
		}
		//on égalise  : $opac_navigateur_bulletin_number de chaque coté
		for($i=0 ; $i<($opac_navigateur_bulletin_number-sizeof($next)) ; $i++){
			if($i){
				...
				if(!$link_perio=get_perio_link($bull['bulletin_notice'],'after'))	$carroussel .="<td style='width:".($taille/(($opac_navigateur_bulletin_number*2)+1))."%;'>&nbsp;</td>";
				else $carroussel .="<td class='active' style='width:".($taille/(($opac_navigateur_bulletin_number*2)+1))."%;'>$link_perio</td>";	
			}
		}
