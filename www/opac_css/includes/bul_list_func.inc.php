...
function affichage_liste_bulletins_normale($res, $print = true) {
	...
	while(($tableau=pmb_mysql_fetch_array($res))) {
		...
		if ($tableau['nbexplnum']>0){
			$padding = "";
			$return.= '<img src="'.get_url_icon('attachment.png').'">';
		}
		else 
...
function affichage_liste_bulletins_tableau($res, $print = true) {
	...
	$return.= "<table cellpadding='2' class='exemplaires' width='100%'>
					<tr>
	...
	while(($tableau=pmb_mysql_fetch_array($res))) {
		...
		$return.= "	<tr $tr_javascript>
						<td>
							<table width='100%'>
								<tr>
									<td style='border:none;width:16px'>".($tableau['nbexplnum'] != 0 ? ($tableau['nbexplnum'] > 1 ? "<img src='".get_url_icon("globe_rouge.png")."' />" : "<img src='".get_url_icon("globe_orange.png")."' />"):"")."</td>
