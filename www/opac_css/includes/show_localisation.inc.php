...
// +-------------------------------------------------+
// $Id: show_localisation.inc.php,v 1.79.2.2 2017-09-05 07:47:03 dgoron Exp $
...
function affiche_notice_navigopac($requete){
	...
	*/	
	...
	print '<div id="navbar"><hr /><center>'.printnavbar($page, $nbr_lignes, $opac_nb_aut_rec_per_page, './index.php?lvl=section_see&id='.$id.'&location='.$location.(($back_surloc)?'&back_surloc='.urlencode($back_surloc):'').(($back_loc)?'&back_loc='.urlencode($back_loc):'').(($back_section_see)?'&back_section_see='.urlencode($back_section_see):'').'&page=!!page!!&nbr_lignes='.$nbr_lignes.'&dcote='.$dcote.'&lcote='.$lcote.'&nc='.$nc.'&main='.$main.'&ssub='.$ssub.'&plettreaut='.$plettreaut.($nb_per_page_custom ? "&nb_per_page_custom=".$nb_per_page_custom : '')).'</center></div>';
...
if (!$location) {
	...
	if (!$id) {
		...
		if (!file_exists($Fnm))	{
			print "<a href='index.php?lvl=section_see&location=".$location.$ajout_back."'><img src='".$image_src."' border='0' align='center' alt='".$msg["l_rayons"]."' title='".$msg["l_rayons"]."'/></a><span class=\"espaceResultSearch\">&nbsp;</span>";
		}
		...
		if($type_aff_navigopac == 0){//Pas de navigation
			...
			if(!$plettreaut){
				...
				foreach ( $tab_aut as $key => $value ) {
					if ($n==0) print "<tr>";
					print "<td width='120px'><a href='./index.php?lvl=section_see&location=".$location."&id=".$id."&plettreaut=".$key.$ajout_back."'><img src='".get_url_icon('folder.gif')."' align='center' border='0'/>".htmlentities($value[1],ENT_QUOTES,$charset)."</a></td>";
					$n++;
					if ($n==$opac_nb_sections_per_line) { print "</tr>"; $n=0; }
				}
				if ($n!=0) {
					...
					if (count($index)>1) {
						...
						if (count($ssub_val)>1) {
							...
							while (list($key,$val)=each($ssub_val)) {
								...
								print "<td width='33%'><a href='./index.php?lvl=section_see&id=".$id."&location=".$location."&dcote=".$val["dcote"]."&lcote=".$val["lcote"]."&nc=".$nc1."&ssub=".$val["ssub"].$ajout_back."'><img src='".get_url_icon('folder.gif')."' align='center' border='0'/>".htmlentities($val["comment"],ENT_QUOTES,$charset)."</a></td>";
								$cur_col++;
